...
public class WM100_CautionMst_01_RenewAndCreate{
	static int SetX;
	static int SetY;
	...
	public static void CautionMstRenewAndCreate(int x,int y,String CautionCd,String DECD,String DepartmentCd) {
		...
		final JTextField TB_CLGpName01		= B100_FrameParts.JTextFieldSet(100, 90,200,20,"",11,0);			//荷主グループ名
		final JTextField TB_DECD			= B100_FrameParts.JTextFieldSet(100,115,100,20,"",11,0);			//届先コード
		final JTextField TB_DepartmentCd	= B100_FrameParts.JTextFieldSet(100,140,100,20,"",11,0);			//部署CD
		final JTextField TB_DEName01		= B100_FrameParts.JTextFieldSet(100,165,200,20,"",11,0);			//届先名
		final JTextField TB_Add01			= B100_FrameParts.JTextFieldSet(100,190,200,20,"",11,0);			//届先住所1
		final JTextField TB_Add02			= B100_FrameParts.JTextFieldSet(100,215,200,20,"",11,0);			//届先住所2
		final JTextField TB_Add03			= B100_FrameParts.JTextFieldSet(100,240,200,20,"",11,0);			//届先住所3
		final JComboBox  TB_CautionTiming	= B100_FrameParts.JComboBoxSet( 100,265,100,20,B100_DefaultVariable.CautionTiming[0],11);	//注意事項タイミング
		final JTextField TB_CautionName		= B100_FrameParts.JTextFieldSet(100,290,150,20,"",11,0);			//注意事項名
		final JTextField TB_Caution			= B100_FrameParts.JTextFieldSet(100,315,300,20,"",11,0);			//注意事項内容
		final JTextField TB_EntryDate		= B100_FrameParts.JTextFieldSet(100,340,200,20,"",11,0);			//データ登録日時
		final JTextField TB_UpdateDate		= B100_FrameParts.JTextFieldSet(100,365,200,20,"",11,0);			//データ更新日時
		final JTextField TB_EntryUser		= B100_FrameParts.JTextFieldSet(100,390,200,20,"",11,0);			//登録者コード
		final JTextField TB_UpdateUser		= B100_FrameParts.JTextFieldSet(100,415,200,20,"",11,0);			//更新者コード
		...
		if(!"".equals(CautionCd)&&!"".equals(DECD)) {
			...
			for(int i=0;i<CautionMstRt.length;i++) {
				if(CautionCd.equals(""+CautionMstRt[i][M100_CautionMstRt.ColCautionCd]) && DECD.equals(""+CautionMstRt[i][M100_CautionMstRt.ColDECD])) {
					for(int i01=0;i01<B100_DefaultVariable.CautionTiming[1].length;i01++) {
						...
						TB_Add01.setText(		""+CautionMstRt[i][M100_CautionMstRt.ColAdd01]);			//届先住所1
		...
		JLabel LB_EntryCom01 = B100_FrameParts.JLabelSet( 10,465,400,20,EntryCom01,11,0);
		JLabel LB_EntryCom02 = B100_FrameParts.JLabelSet( 10,485,400,20,EntryCom02,11,0);
		JLabel LB_EntryCom03 = B100_FrameParts.JLabelSet( 10,510,400,20,EntryCom03,11,0);
		JLabel LB_EntryCom04 = B100_FrameParts.JLabelSet( 10,530,400,20,EntryCom04,11,0);
		...
		//荷主Grp検索ボタン
		JButton ClGrpSearchBtn = B100_FrameParts.BtnSet(210, 65,100,20,"荷主Grp検索",11);
		main_fm.add(ClGrpSearchBtn);
		
		//届先検索ボタン
		JButton DeliverySearchBtn = B100_FrameParts.BtnSet(210,115,100,20,"届先検索",11);
		...
		tb_ClGrp.setRowHeight(20*A00000_Main.Mul/A00000_Main.Div);
		tb_ClGrp.setFont(new Font(A00000_Main.DefaultFont, Font.PLAIN, 12*A00000_Main.Mul/A00000_Main.Div));
		...
		JButton ClGrpSearch_Btn = B100_FrameParts.BtnSet(100,100,100,20,"検索",11);
		ClGrpSearch_fm.add(ClGrpSearch_Btn);
		
		JButton ClGrpEntry_Btn = B100_FrameParts.EntryBtn();
		...
		ClGrpSearch_Btn.addActionListener(new AbstractAction(){
			public void actionPerformed(ActionEvent e){
				if(RenewFg) {
					...
					GetSearchGrpName	= B100_TextControl.Trim(GetSearchGrpName);
					GetSearchGrpAdd		= B100_TextControl.Trim(GetSearchGrpAdd);
					GetSearchGrpTel		= B100_TextControl.Trim(GetSearchGrpTel);
					
					GetSearchGrpTel		= B100_TextControl.num_only_String(GetSearchGrpTel);
		...
		tb_Delivery.setRowHeight(20*A00000_Main.Mul/A00000_Main.Div);
		tb_Delivery.setFont(new Font(A00000_Main.DefaultFont, Font.PLAIN, 12*A00000_Main.Mul/A00000_Main.Div));
		...
		JButton DeliverySearch_Btn = B100_FrameParts.BtnSet(100,100,100,20,"検索",11);
		...
		DeliverySearch_Btn.addActionListener(new AbstractAction(){
			public void actionPerformed(ActionEvent e){
				if(RenewFg) {
					...
					GetSearchDeliveryName	= B100_TextControl.Trim(GetSearchDeliveryName);
					GetSearchDeliveryAdd	= B100_TextControl.Trim(GetSearchDeliveryAdd);
					GetSearchDeliveryTel	= B100_TextControl.Trim(GetSearchDeliveryTel);
					
					GetSearchDeliveryTel	= B100_TextControl.num_only_String(GetSearchDeliveryTel);
					...
					if(!"".equals(GetDECD)) {
						...
						if(0<DeliveryMstRt.length) {
							if("".equals(GetDepartmentCd)) {
								...
								for(int i=0;i<DeliveryMstRt.length;i++) {
									if(GetDepartmentCd.equals(""+DeliveryMstRt[i][M100_DeliveryMstRt.ColDepartmentCd])) {
										TB_DEName01.setText(""+DeliveryMstRt[i][M100_DeliveryMstRt.ColDEName01]);
										TB_Add01.setText(""+DeliveryMstRt[i][M100_DeliveryMstRt.ColAdd01]);
										TB_Add02.setText(""+DeliveryMstRt[i][M100_DeliveryMstRt.ColAdd02]);
										TB_Add03.setText(""+DeliveryMstRt[i][M100_DeliveryMstRt.ColAdd03]);
										UnHitFg=false;
									}
								}
								if(UnHitFg) {
		...
		entry_btn.addActionListener(new AbstractAction(){
			public void actionPerformed(ActionEvent e){
				...
				String GetCautionTiming	= B100_DefaultVariable.CautionTiming[1][TB_CautionTiming.getSelectedIndex()];	//注意事項タイミング
				...
				GetCautionCd		= B100_TextControl.Trim(GetCautionCd);
				GetClGpCD			= B100_TextControl.Trim(GetClGpCD);
				GetDECD				= B100_TextControl.Trim(GetDECD);
				GetDepartmentCd		= B100_TextControl.Trim(GetDepartmentCd);
				GetCautionTiming	= B100_TextControl.Trim(GetCautionTiming);
				GetCautionName		= B100_TextControl.Trim(GetCautionName);
				GetCaution			= B100_TextControl.Trim(GetCaution);
				
				if(!"".equals(GetDECD)) {
					if("".equals(GetCautionCd)) {GetCautionCd = M100_CautionMstRt.NewCautionCdGet(1)[0];}
