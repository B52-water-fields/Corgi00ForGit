...
public class WM100_ShippingCompanyMst_00_Search{
	static int SetX;
	static int SetY;
	...
	public static void ShippingCompanyMstSearch(int x,int y) {
		...
		if(y==0) {y=SetY;}
		RenewFg = false;
		final JFrame main_fm = B100_FrameParts.FrameCreate(x,y,850,750,"Corgi00運送会社検索　WM100_ShippingCompanyMst_00_Search","");
		...
		final JTextField TB_SearchPost				= B100_FrameParts.JTextFieldSet(	100, 75,100,20,"",11,0);	//郵便番号
		final JTextField TB_SearchAdd				= B100_FrameParts.JTextFieldSet(	100,100,100,20,"",11,0);	//住所
		final JTextField TB_SearchTel				= B100_FrameParts.JTextFieldSet(	350, 25,100,20,"",11,0);	//Tel
		final JTextField TB_SearchFax				= B100_FrameParts.JTextFieldSet(	350, 50,100,20,"",11,0);	//Fax
		...
		JLabel LB2_SearchShippingCompanyCd	= B100_FrameParts.JLabelSet(200, 25, 50,20,"と一致"	,11,0);
		JLabel LB2_SearchCompanyName		= B100_FrameParts.JLabelSet(200, 50, 50,20,"を含む"	,11,0);
		JLabel LB2_SearchPost				= B100_FrameParts.JLabelSet(200, 75, 50,20,"で始まる"	,11,0);
		JLabel LB2_SearchAdd				= B100_FrameParts.JLabelSet(200,100, 50,20,"を含む"	,11,0);
		...
		for(int i=0;i<RtSettingShippingCompanyMstRt.length;i++) {
			columnNames01[1+(int)RtSettingShippingCompanyMstRt[i][1]] = ""+RtSettingShippingCompanyMstRt[i][3];
		}
		...
		for(int i=0;i<RtSettingShippingCompanyMstRt.length;i++) {
			if("int".equals((String)RtSettingShippingCompanyMstRt[i][2])||"float".equals((String)RtSettingShippingCompanyMstRt[i][2])) {
				column = columnModel01.getColumn(1+(int)RtSettingShippingCompanyMstRt[i][1]);	column.setPreferredWidth( 90*A00000_Main.Mul/A00000_Main.Div);	column.setCellRenderer(B100_FrameParts.rightCellRenderer());
			}else {
				column = columnModel01.getColumn(1+(int)RtSettingShippingCompanyMstRt[i][1]);	column.setPreferredWidth( 90*A00000_Main.Mul/A00000_Main.Div);	column.setCellRenderer(B100_FrameParts.leftCellRenderer());
			}
		}
		
		//スクロール用設定
		JScrollPane scpn01 = B100_FrameParts.JScrollPaneSet(10,210,820,425,tb01);
		main_fm.add(scpn01);
		...
		SearchBtn.addActionListener(new AbstractAction(){
			public void actionPerformed(ActionEvent e){
				if(RenewFg) {
					...
					GetSearchShippingCompanyCd	= B100_TextControl.Trim(GetSearchShippingCompanyCd);
					GetSearchCompanyName		= B100_TextControl.Trim(GetSearchCompanyName);
			...
			public void actionPerformed(ActionEvent e){
				if(RenewFg) {
					...
					if(!"".equals(TgtShippingCompanyCd)) {
						...
						main_fm.dispose();
						WM100_ShippingCompanyMst_01_RenewAndCreate.ShippingCompanyMstRenewAndCreate(0,0,TgtShippingCompanyCd);
					}
					RenewFg = true;
				}
			}
		...
		CreateBtn.addActionListener(new AbstractAction(){
			public void actionPerformed(ActionEvent e){
				...
				main_fm.setVisible(false);
				main_fm.dispose();
				WM100_ShippingCompanyMst_01_RenewAndCreate.ShippingCompanyMstRenewAndCreate(0,0,"");
			}
		});
		...
		CsvBtn.addActionListener(new AbstractAction(){
			public void actionPerformed(ActionEvent e){
				if(RenewFg) {
					RenewFg = false;
					B100_TableControl.TableOutPutCsv("出力先選択","運送会社マスタ検索結果",tb01);
					RenewFg = true;
				}
			}
		});
		...
		ExcelBtn.addActionListener(new AbstractAction(){
			public void actionPerformed(ActionEvent e){
				if(RenewFg) {
					RenewFg = false;
					B100_TableControl.TableOutPutExcel("出力先選択","運送会社マスタ検索結果",tb01);
					RenewFg = true;
				}
			}
		});
