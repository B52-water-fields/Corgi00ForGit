...
public class WM100_SupplierMst_02_ExcelEntry{
	static int SetX;
	static int SetY;
	...
	private static void MstEntry(Object[][] CheckOb,String[] TableCol) {
		...
		for(int i=0;i<SetObRt.length;i++) {
			switch( (String)SetObRt[i][0]) {
				case "SPCd"			:
					...
					String[] SetSpCd	= M100_SupplierRt.NewSpCdGet(NeedCount);
					NeedCount = 0;
					for(int i01=0;i01<((String[])SetObRt[i][4]).length;i01++) {
						if("".equals(((String[])SetObRt[i][4])[i01])) {
		...
		A100_InsertUpdateSQL.InsertUpdateSomeRecord(SetObRt,tgt_table,TgtDB,non_msg_fg);
			...
			switch( (String)SetObRt[i][0]) {
				case "ClWh"			:
					for(int i01=0;i01<((String[])SetObRt[i][4]).length;i01++){
						...
						if(A00000_Main.ClWh.equals(((String[])SetObRt[i][4])[i01])) {
							CheckFg = true;
						}
				...
				case "ClCd"			:
					for(int i01=0;i01<((String[])SetObRt[i][4]).length;i01++){
						...
						if(A00000_Main.ClCd.equals(((String[])SetObRt[i][4])[i01])) {
							CheckFg = true;
						}
				...
				case "PayDate"		:
					for(int i01=0;i01<((String[])SetObRt[i][4]).length;i01++){
						if(null==((String[])SetObRt[i][4])[i01]) {((String[])SetObRt[i][4])[i01]="";}
						boolean CheckFg = B100_DefaultVariableCompare.ShimeDateListCheck(((String[])SetObRt[i][4])[i01]);
	...
	private static Object[][] SetObRt(Object[][] CheckOb,String[] TableCol) {
		...
		for(int i=0;i<CheckOb.length;i++) {
			...
			if(!SkipFg) {
				...
				SetEntryUser[EntryCount]		= "(" + A00000_Main.LoginUserId + ")" + A00000_Main.LoginUserName;
				SetUpdateUser[EntryCount]		= "(" + A00000_Main.LoginUserId + ")" + A00000_Main.LoginUserName;
