...
public class WM100_ItemMst_02_ExcelEntry{
	static int SetX;
	static int SetY;
	...
	public static void ItemMstExcelEntryMain(int x,int y,String TgtFilePath,String SheetName) {
		...
		if(y==0) {y=SetY;}
		RenewFg = false;
		final JFrame main_fm = B100_FrameParts.FrameCreate(x,y,850,800,"Corgi00商品マスタ登録（エクセル）","");
		JLabel userinfo = B100_FrameParts.UserInfo();
		JButton exit_btn = B100_FrameParts.ExitBtn();
		JButton entry_btn = B100_FrameParts.EntryBtn();
		...
		JScrollPane scpn01 = B100_FrameParts.JScrollPaneSet(10,65,800,600,tb01);
		main_fm.add(scpn01);
	...
	private static void DataEntry(Object[][] CheckOb,String[]TableCol) {
		...
		for(int i01=0;i01<CheckOb.length;i01++) {
			for(int i02=0;i02<CheckOb[i01].length;i02++) {
				CheckOb[i01][i02] = B100_TextControl.Trim(""+CheckOb[i01][i02]);
			}
			//商品CD・商品名が空白ならSKIP
			if("".equals(""+CheckOb[i01][ColItemCd])
				...
				GetExpDateHowLong[EntryCount] 		= B100_TextControl.num_only_String02(GetExpDateHowLong[EntryCount]);	//賞味期限日数
				GetCtQty[EntryCount] 				= B100_TextControl.num_only_String02(GetCtQty[EntryCount]);				//カートン入数
				GetCsQty[EntryCount] 				= B100_TextControl.num_only_String02(GetCsQty[EntryCount]);				//ケース入数
				GetPlQty[EntryCount] 				= B100_TextControl.num_only_String02(GetPlQty[EntryCount]);				//パレット入数
				GetItemWeight[EntryCount] 			= B100_TextControl.num_only_String02(GetItemWeight[EntryCount]);		//商品重量
				GetCtWeight[EntryCount] 			= B100_TextControl.num_only_String02(GetCtWeight[EntryCount]);			//カートン重量
				GetCsWeight[EntryCount] 			= B100_TextControl.num_only_String02(GetCsWeight[EntryCount]);			//ケース重量
				GetPlWeight[EntryCount] 			= B100_TextControl.num_only_String02(GetPlWeight[EntryCount]);			//パレット重量
				GetItemSize[EntryCount] 			= B100_TextControl.num_only_String02(GetItemSize[EntryCount]);			//商品サイズ
				GetCtSize[EntryCount] 				= B100_TextControl.num_only_String02(GetCtSize[EntryCount]);			//カートンサイズ
				GetCsSize[EntryCount] 				= B100_TextControl.num_only_String02(GetCsSize[EntryCount]);			//ケースサイズ
				GetPlSize[EntryCount] 				= B100_TextControl.num_only_String02(GetPlSize[EntryCount]);			//パレットサイズ
				...
				for(int i02 =0;i02<B100_DefaultVariable.TildFG[1].length;i02++) {
					if(GetTildFG[EntryCount].equals(B100_DefaultVariable.TildFG[1][i02])) {
						GetTildName[EntryCount] = B100_DefaultVariable.TildFG[2][i02];
					}
				}
				EntryCount = EntryCount+1;
			}
		...
		for(int i01=0;i01<EntryCount;i01++) {
			if(!"".equals(GetItemImagePath01[i01])){		//画像パス01 
				String SavePath = A00000_Main.FileFldPth+"\\ItemImage\\"+GetClGpCd[i01];
				String FullSavePath = SavePath+"\\"+GetItemCd[i01]+"_01.";
				String GetItemImagePath = GetItemImagePath01[i01];
				
				if(B100_PictControl.PictCheck(GetItemImagePath)&&B100_FolderCheck.FLD_CHECK_ONRY(SavePath)) {
					String FileType=B100_FolderCheck.FILE_TYPE(GetItemImagePath);
					Image image = B100_PictControl.PictReSize(1024,1024,GetItemImagePath);
					FullSavePath = FullSavePath+FileType;
					B100_PictControl.PictSave(image,FileType,FullSavePath);
			...
			if("".equals(GetItemImagePath01[i01])||"".equals(GetItemImagePath02[i01])||"".equals(GetItemImagePath03[i01])||"".equals(GetItemImagePath04[i01])||"".equals(GetItemImagePath05[i01])) {
				for(int i02=0;i02<ItemMstRt.length;i02++) {
					if(GetClGpCd[i01].equals(""+ItemMstRt[i02][M100_ItemMstRt.ColClGpCd])&&GetItemCd[i01].equals(""+ItemMstRt[i02][M100_ItemMstRt.ColItemCd])) {
						if("".equals(GetItemImagePath01[i01])) {
							GetItemImagePath01[i01] = ""+ItemMstRt[i02][M100_ItemMstRt.ColPictPass01];
						}
						if("".equals(GetItemImagePath02[i01])) {
							GetItemImagePath02[i01] = ""+ItemMstRt[i02][M100_ItemMstRt.ColPictPass02];
						}
						if("".equals(GetItemImagePath03[i01])) {
							GetItemImagePath03[i01] = ""+ItemMstRt[i02][M100_ItemMstRt.ColPictPass03];
						}
						if("".equals(GetItemImagePath04[i01])) {
							GetItemImagePath04[i01] = ""+ItemMstRt[i02][M100_ItemMstRt.ColPictPass04];
						}
						if("".equals(GetItemImagePath05[i01])) {
							GetItemImagePath05[i01] = ""+ItemMstRt[i02][M100_ItemMstRt.ColPictPass05];
						}
		...
		for(int i01=0;i01<EntryCount;i01++) {
			...
			entry_data[i01][15] = "(" + A00000_Main.LoginUserId + ")" + A00000_Main.LoginUserName;	//登録者コード
			entry_data[i01][16] = "(" + A00000_Main.LoginUserId + ")" + A00000_Main.LoginUserName;	//更新者コード
		...
		for(int i01=0;i01<EntryCount;i01++) {
			...
			entry_data[i01][33] = "(" + A00000_Main.LoginUserId + ")" + A00000_Main.LoginUserName;	//登録者コード
			entry_data[i01][34] = "(" + A00000_Main.LoginUserId + ")" + A00000_Main.LoginUserName;	//更新者コード
	...
	private static ArrayList<String> ErrCheck(Object[][] CheckOb,String[]TableCol) {
		...
		for(int i=0;i<CheckOb.length;i++) {
			...
			for(int i01=0;i01<CheckOb[i].length;i01++) {
				CheckOb[i][i01] = B100_TextControl.Trim(""+CheckOb[i][i01]);
			}
			if("".equals(""+CheckOb[i][ColItemCd])
					&& "".equals(""+CheckOb[i][ColItemName01])
					) {
				...
				for(int i01=0;i01<ClGpMstRt.length;i01++) {
					if((""+CheckOb[i][ColClGpCd]).equals(""+ClGpMstRt[i01][M100_ClGpMstRt.ColClGpCD])) {
						UnHitFg = false;
				...
				UnHitFg = true;
				for(int i01=0;i01<B100_DefaultVariable.DeliveryType01[1].length;i01++) {
					if((""+CheckOb[i][ColDeliveryTypeCd01]).equals(B100_DefaultVariable.DeliveryType01[1][i01])) {
						UnHitFg = false;
						i01=B100_DefaultVariable.DeliveryType01[1].length+1;
					}
				}
				if(UnHitFg) {
					int wint=i+1;
				...
				UnHitFg = true;
				for(int i01=0;i01<B100_DefaultVariable.DeliveryType02[1].length;i01++) {
					if((""+CheckOb[i][ColDeliveryTypeCd02]).equals(B100_DefaultVariable.DeliveryType02[1][i01])) {
						UnHitFg = false;
						i01=B100_DefaultVariable.DeliveryType02[1].length+1;
					}
				}
				if(UnHitFg) {
					int wint=i+1;
				...
				UnHitFg = true;
				for(int i01=0;i01<B100_DefaultVariable.DeliveryType03[1].length;i01++) {
					if((""+CheckOb[i][ColDeliveryTypeCd03]).equals(B100_DefaultVariable.DeliveryType03[1][i01])) {
						UnHitFg = false;
						i01=B100_DefaultVariable.DeliveryType03[1].length+1;
					}
				}
				if(UnHitFg) {
					int wint=i+1;
				...
				UnHitFg = true;
				for(int i01=0;i01<B100_DefaultVariable.DeliveryType04[1].length;i01++) {
					if((""+CheckOb[i][ColDeliveryTypeCd04]).equals(B100_DefaultVariable.DeliveryType04[1][i01])) {
						UnHitFg = false;
						i01=B100_DefaultVariable.DeliveryType04[1].length+1;
					}
				}
				if(UnHitFg) {
					int wint=i+1;
				...
				UnHitFg = true;
				for(int i01=0;i01<B100_DefaultVariable.DeliveryType05[1].length;i01++) {
					if((""+CheckOb[i][ColDeliveryTypeCd05]).equals(B100_DefaultVariable.DeliveryType05[1][i01])) {
						UnHitFg = false;
						i01=B100_DefaultVariable.DeliveryType05[1].length+1;
					}
				}
				if(UnHitFg) {
					int wint=i+1;
				...
				UnHitFg = true;
				for(int i01=0;i01<B100_DefaultVariable.TildFG[1].length;i01++) {
					if((""+CheckOb[i][ColTildFG]).equals(B100_DefaultVariable.TildFG[1][i01])) {
						UnHitFg = false;
						i01=B100_DefaultVariable.TildFG[1].length+1;
					}
				}
				if(UnHitFg) {
					int wint=i+1;
				...
				for(int i01=0;i01<B100_DefaultVariable.DelList[1].length;i01++) {
					if((""+CheckOb[i][ColDelFg]).equals(B100_DefaultVariable.DelList[1][i01])) {
						UnHitFg = false;
						i01=B100_DefaultVariable.DelList[1].length+1;
					}
				}
				if(UnHitFg) {
					int wint=i+1;
	...
	private static void ErrView(ArrayList<String>ErrMsg) {
		...
		FLD_PATH = A00000_Main.MainFLD+"\\MstControl\\ItemMst";
		B100_FolderCheck.FLD_CHECK(FLD_PATH);
		FLD_PATH = A00000_Main.MainFLD+"\\MstControl\\ItemMst\\Err";
		B100_FolderCheck.FLD_CHECK(FLD_PATH);
		FLD_PATH = A00000_Main.MainFLD+"\\MstControl\\ItemMst\\BK";
		B100_FolderCheck.FLD_CHECK(FLD_PATH);
