...
public class WM100_AdjustReasonMst_01_RenewAndCreate{
	static int SetX;
	static int SetY;
	...
	public static void AdjustReasonMstRenewAndCreate(int x,int y,String TgtClCd,String TgtWhCd,String TgtAdjustReasonCd) {
		...
		final JTextField  TB_EntryUser			= B100_FrameParts.JTextFieldSet( 130,200,300,20,"",11,0);	//登録者
		final JTextField  TB_UpdateUser			= B100_FrameParts.JTextFieldSet( 130,225,300,20,"",11,0);	//更新者
		...
		for(int i=0;i<B100_DefaultVariable.ClList[1].length;i++) {
			if(B100_DefaultVariable.ClList[1][i].equals(TgtClCd)) {
				TB_ClCd.setSelectedIndex(i);
			}
		}
		
		for(int i=0;i<B100_DefaultVariable.WhList[1].length;i++) {
			if(B100_DefaultVariable.WhList[1][i].equals(TgtWhCd)) {
				TB_ClWh.setSelectedIndex(i);
			}
		}
	...
	private static String[] TxtTrim(
		...
		GetAdjustReasonCd	= B100_TextControl.Trim(GetAdjustReasonCd);
		GetAdjustReasonName	= B100_TextControl.Trim(GetAdjustReasonName);
	...
	private static String DataEntry(
		...
		if("".equals(GetAdjustReasonCd)) {GetAdjustReasonCd=M100_AdjustReasonRt.NewAdjustReasonCdGet(1)[0];}
		...
		B100_DefaultVariable.AdjustReasonDefault();
	...
	private static void ErrView(ArrayList<String>ErrMsg) {
		...
		FLD_PATH = A00000_Main.MainFLD+"\\MstControl\\AdjustReasonMst";
		B100_FolderCheck.FLD_CHECK(FLD_PATH);
		FLD_PATH = A00000_Main.MainFLD+"\\MstControl\\AdjustReasonMst\\Err";
		B100_FolderCheck.FLD_CHECK(FLD_PATH);
		FLD_PATH = A00000_Main.MainFLD+"\\MstControl\\AdjustReasonMst\\BK";
		B100_FolderCheck.FLD_CHECK(FLD_PATH);
