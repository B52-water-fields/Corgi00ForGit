...
public class M100_SupplierRt{
	...
	Object[][] SupplierRt = M100_SupplierRt.SupplierRt(
			...
			String GetSPAdd03		=	(String)SupplierRt[i][M100_SupplierRt.ColSPAdd03];			//仕入先住所3
			String GetSPTel			=	(String)SupplierRt[i][M100_SupplierRt.ColSPTel];			//仕入先電話
			String GetSPFax			=	(String)SupplierRt[i][M100_SupplierRt.ColSPFax];			//仕入先FAX
			String GetSPMail		=	(String)SupplierRt[i][M100_SupplierRt.ColSPMail];			//仕入先MAIL
			String GetCom01			=	(String)SupplierRt[i][M100_SupplierRt.ColCom01];			//コメント1
			String GetCom02			=	(String)SupplierRt[i][M100_SupplierRt.ColCom02];			//コメント2
	...
	*/
	...
	static final int ColSPAdd03			= (int)11;	//仕入先住所3
	static final int ColSPTel				= (int)12;	//仕入先電話
	static final int ColSPFax				= (int)13;	//仕入先FAX
	static final int ColSPMail				= (int)14;	//仕入先MAIL
	static final int ColCom01				= (int)15;	//コメント1
	static final int ColCom02				= (int)16;	//コメント2
	static final int ColCom03				= (int)17;	//コメント3
	static final int ColPTMSCDBMN			= (int)18;	//基幹Sysコード（部門）
	static final int ColPTMSCDNINUSHI		= (int)19;	//基幹Sysコード（荷主）
	static final int ColPaySite			= (int)20;	//支払いサイト（月数）
	static final int ColPayDate			= (int)21;	//支払日　末日＝99
	static final int ColShimeDate			= (int)22;	//締め日　末日＝99
	...
	public static Object[][] SupplierRt(
		...
		String sql = "select "
				...
				+ " from "+A00000_Main.MySqlDefaultSchemaWANKO+".WM0010Supplier"
	...
	public static String[] NewSpCdGet(int NeedCount) {
    	...
    	for(int i=0;i<SupplierRt.length;i++) {
    		if(4<(""+SupplierRt[i][M100_SupplierRt.ColSPCd]).length()&&"ATSP".equals((""+SupplierRt[i][M100_SupplierRt.ColSPCd]).substring(0,4))) {
    			String WST = B100_TextControl.num_only_String(""+SupplierRt[i][M100_SupplierRt.ColSPCd]);
    			if("".equals(WST)){WST = "0";}
