...
public class WM100_LocationMst_00_Search{
	static int SetX;
	static int SetY;
	...
	public static void LocationMstSearch(int x,int y) {
		...
		if(y==0) {y=SetY;}
		RenewFg = false;
		final JFrame main_fm = B100_FrameParts.FrameCreate(x,y,900,750,"Corgi00ロケーションマスタ検索　WM100_LocationMst_00_Search","");
		...
		final JTextField  TB_SearchLoc		= B100_FrameParts.JTextFieldSet(100, 75,100,20,"",11,0);										//ロケーション
		...
		for(int i=0;i<B100_DefaultVariable.SearchWhList[1].length;i++) {
			if(A00000_Main.ClWh.equals(B100_DefaultVariable.SearchWhList[1][i])) {
		...
		//スクロール用設定
		JScrollPane scpn01 = B100_FrameParts.JScrollPaneSet(10,230,870,395,tb01);
		main_fm.add(scpn01);
		...
		SearchBtn.addActionListener(new AbstractAction(){
			public void actionPerformed(ActionEvent e){
				if(RenewFg) {
					...
					GetSearchLoc 		= B100_TextControl.Trim(GetSearchLoc);
					GetSearchLocName 	= B100_TextControl.Trim(GetSearchLocName);
					GetSearchType 		= B100_TextControl.Trim(GetSearchType);
				...
				if(RenewFg) {
					...
					for(int i=0;i<RowCount;i++) {
						if((boolean)MainFmTableModel.getValueAt(i, 0)) {
							TgtClCd = ""+MainFmTableModel.getValueAt(i, M100_LocationMstRt.ColClCd+1);
							TgtWhCd = ""+MainFmTableModel.getValueAt(i, M100_LocationMstRt.ColWhCd+1);
							TgtLoc 	= ""+MainFmTableModel.getValueAt(i, M100_LocationMstRt.ColLoc+1);
							KickFg = true;
						}
					}
					if(KickFg) {
						...
						main_fm.dispose();
						WM100_LocationMst_01_RenewAndCreate.LocationMstRenewAndCreate(0,0,TgtClCd,TgtWhCd,TgtLoc);
					}
					RenewFg = true;
				}
			}
			...
			public void actionPerformed(ActionEvent e){
				if(RenewFg) {
					...
					if(null!=Selected && !Selected.equals(Selected.replace(".xlsx", ""))) {
						...
						main_fm.setVisible(false);
						main_fm.dispose();
						WM100_LocationMst_02_ExcelEntry.LocationMstExcelEntry(0,0,Selected);
					}
					RenewFg = true;
				}
			}
		...
		CsvBtn.addActionListener(new AbstractAction(){
			public void actionPerformed(ActionEvent e){
				if(RenewFg) {
					RenewFg = false;
					B100_TableControl.TableOutPutCsv("出力先選択","ロケーションマスタ検索結果",tb01);
					RenewFg = true;
				}
			}
		});
		...
		ExcelBtn.addActionListener(new AbstractAction(){
			public void actionPerformed(ActionEvent e){
				if(RenewFg) {
					RenewFg = false;
					B100_TableControl.TableOutPutExcel("出力先選択","ロケーションマスタ検索結果",tb01);
					RenewFg = true;
				}
			}
		});
