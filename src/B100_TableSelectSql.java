public class B100_TableSelectSql{
	...
	public static void TableSelectSqlCore(String TgtDB,String TgtTable) {
		...
		String[][] ColumnList = A100_OldDataTableCheck.ColumnList(TgtDB,TgtTable);
			...
			for(int i=0;i<ColumnList.length;i++) {
				switch(ColumnList[i][ 7]) {
					...
					case "int":
						...
						break;
					case "datetime":
						System.out.println("if(null==rset01.getTimestamp(\""+ColumnList[i][ 3]+"\")){rt[counter]["+i+"]=\"\";}else{rt[counter]["+i+"]=B100_DateTimeControl.dtmString2(rset01.getTimestamp(\""+ColumnList[i][ 3]+"\"))[1];}");
						break;
					case "float":
