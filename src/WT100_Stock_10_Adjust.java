	...
	public static void StockAdjust(int x,int y,final String HandoverWhCd,final String HandoverClCd,final String HandoverLoc,final String HandoverItemCd,final String HandoverLot,final String HandoverExpdate,final String HandoverActualDate) {
		...
		******************************************/
		...
		if(!"".equals(TgtWhCd)
			...
			if(1==StockRt.length) {
				GetClCd				= (String)StockRt[0][T100_StockRt.ColClCd];			//荷主コード
				GetCLName			= (String)StockRt[0][T100_StockRt.ColCLName];			//荷主名1
				GetWhCd				= (String)StockRt[0][T100_StockRt.ColWhCd];			//倉庫コード
				GetClWHName			= (String)StockRt[0][T100_StockRt.ColClWHName];		//担当倉庫名
				GetClGpCD			= (String)StockRt[0][T100_StockRt.ColClGpCD];			//荷主グループCD
				GetClGpName			= (String)StockRt[0][T100_StockRt.ColClGpName];		//グループ名1
				GetLoc				= (String)StockRt[0][T100_StockRt.ColLoc];			//ロケーション
				GetLocName			= (String)StockRt[0][T100_StockRt.ColLocName];		//ロケーション名
				GetType				= (int)StockRt[0][T100_StockRt.ColType];				//ロケタイプ
				GetItemCd			= (String)StockRt[0][T100_StockRt.ColItemCd];			//商品コード
				GetLot				= (String)StockRt[0][T100_StockRt.ColLot];			//ロット
				GetExpdate			= (String)StockRt[0][T100_StockRt.ColExpdate];		//消費期限
				GetActualDate		= (String)StockRt[0][T100_StockRt.ColActualDate];	//入荷実績日
				GetQty				= (int)StockRt[0][T100_StockRt.ColQty];				//総数量
				GetShipPlanQty		= (int)StockRt[0][T100_StockRt.ColShipPlanQty];		//引当済総数
				GetPossibleQty		= (int)StockRt[0][T100_StockRt.ColPossibleQty];		//出荷可能総数
				GetItemName			= (String)StockRt[0][T100_StockRt.ColItemName];		//商品名
				GetItemName01		= (String)StockRt[0][T100_StockRt.ColItemName01];	//商品名1
				GetItemName02		= (String)StockRt[0][T100_StockRt.ColItemName02];	//商品名2
				GetItemName03		= (String)StockRt[0][T100_StockRt.ColItemName03];	//商品名3
				GetClItemCd			= (String)StockRt[0][T100_StockRt.ColClItemCd];		//荷主商品コード
				GetJanCd			= (String)StockRt[0][T100_StockRt.ColJanCd];			//ソースマーク_BCD（バラ）
				GetItemMdNo			= (String)StockRt[0][T100_StockRt.ColItemMdNo];		//商品型番
				GetCtUnitQty		= (int)StockRt[0][T100_StockRt.ColCtUnitQty];		//カートン入数
				GetCsUnitQty		= (int)StockRt[0][T100_StockRt.ColCsUnitQty];		//ケース入数
				GetPlUnitQty		= (int)StockRt[0][T100_StockRt.ColPlUnitQty];		//パレット入数
				GetUnitName			= (String)StockRt[0][T100_StockRt.ColUnitName];		//商品単位
				GetCtUnitName		= (String)StockRt[0][T100_StockRt.ColCtUnitName];	//カートン商品単位
				GetCsUnitName		= (String)StockRt[0][T100_StockRt.ColCsUnitName];	//ケース商品単位
				GetPlUnitName		= (String)StockRt[0][T100_StockRt.ColPlUnitName];	//パレット商品単位
				GetEntryDate		= (String)StockRt[0][T100_StockRt.ColEntryDate];		//登録日時
				GetUpdateDate		= (String)StockRt[0][T100_StockRt.ColUpdateDate];	//更新日時
				GetEntryUser		= (String)StockRt[0][T100_StockRt.ColEntryUser];		//登録者
				GetUpdateUser		= (String)StockRt[0][T100_StockRt.ColUpdateUser];	//更新者
				GetBrQty			= (int)StockRt[0][T100_StockRt.ColBrQty];				//バラ数量
				GetBrShipPlanQty	= (int)StockRt[0][T100_StockRt.ColBrShipPlanQty];	//引当済バラ数
				GetBrPossibleQty	= (int)StockRt[0][T100_StockRt.ColBrPossibleQty];	//出荷可能バラ数
				GetCtQty			= (int)StockRt[0][T100_StockRt.ColCtQty];				//カートン数量
				GetCtShipPlanQty	= (int)StockRt[0][T100_StockRt.ColCtShipPlanQty];	//引当済カートン数
				GetCtPossibleQty	= (int)StockRt[0][T100_StockRt.ColCtPossibleQty];	//出荷可能カートン数
				GetCsQty			= (int)StockRt[0][T100_StockRt.ColCsQty];				//ケース数量
				GetCsShipPlanQty	= (int)StockRt[0][T100_StockRt.ColCsShipPlanQty];	//引当済ケース数
				GetCsPossibleQty	= (int)StockRt[0][T100_StockRt.ColCsPossibleQty];	//出荷可能ケース数
				GetPlQty			= (int)StockRt[0][T100_StockRt.ColPlQty];				//パレット数量
				GetPlShipPlanQty	= (int)StockRt[0][T100_StockRt.ColPlShipPlanQty];	//引当済パレット数
				GetPlPossibleQty	= (int)StockRt[0][T100_StockRt.ColPlPossibleQty];	//出荷可能パレット数
		...
		if(MstCheckFg) {
			...
			if(1==ClMstRt.length) {
				GetClCd			=(String)ClMstRt[0][M100_ClMstRt.Colcl_cd];		//荷主CD
				GetCLName		=(String)ClMstRt[0][M100_ClMstRt.ColCLName01];	//荷主名1
				GetClGpCD		=(String)ClMstRt[0][M100_ClMstRt.ColClGpCD];		//荷主グループCD
				GetClGpName		=(String)ClMstRt[0][M100_ClMstRt.ColClGpName];	//グループ名1
			}
			
			//倉庫マスタ
			Object[][] WhMstRt = WhMstRt(GetWhCd);
			if(1==WhMstRt.length){
				GetWhCd				= (String)WhMstRt[0][M100_WhMstRt.ColNoWHCD];			//倉庫コード
				GetClWHName			= (String)WhMstRt[0][M100_WhMstRt.ColNoWHName];		//担当倉庫名
			}
			
			//商品マスタ
			Object[][] ItemMstRt = ItemMstRt(GetClGpCD,GetItemCd);
			if(1==ItemMstRt.length) {
				GetItemCd			= (String)ItemMstRt[0][M100_ItemMstRt.ColItemCd];			//商品コード
				GetItemName			= (String)ItemMstRt[0][M100_ItemMstRt.ColItemName01];	//商品名
				GetItemName01		= (String)ItemMstRt[0][M100_ItemMstRt.ColItemName01];	//商品名1
				GetItemName02		= (String)ItemMstRt[0][M100_ItemMstRt.ColItemName02];	//商品名2
				GetItemName03		= (String)ItemMstRt[0][M100_ItemMstRt.ColItemName03];	//商品名3
				GetClItemCd			= (String)ItemMstRt[0][M100_ItemMstRt.ColCLItemCd];		//荷主商品コード
				GetJanCd			= (String)ItemMstRt[0][M100_ItemMstRt.ColJanCd];			//ソースマーク_BCD（バラ）
				GetItemMdNo			= (String)ItemMstRt[0][M100_ItemMstRt.ColItemMDNo];		//商品型番
				GetCtUnitQty		= (int)ItemMstRt[0][M100_ItemMstRt.ColCtQty];				//カートン入数
				GetCsUnitQty		= (int)ItemMstRt[0][M100_ItemMstRt.ColCsQty];				//ケース入数
				GetPlUnitQty		= (int)ItemMstRt[0][M100_ItemMstRt.ColPlQty];				//パレット入数
				GetUnitName			= (String)ItemMstRt[0][M100_ItemMstRt.ColUnitName];		//商品単位
				GetCtUnitName		= (String)ItemMstRt[0][M100_ItemMstRt.ColCtUnitName];	//カートン商品単位
				GetCsUnitName		= (String)ItemMstRt[0][M100_ItemMstRt.ColCsUnitName];	//ケース商品単位
				GetPlUnitName		= (String)ItemMstRt[0][M100_ItemMstRt.ColPlUnitName];	//パレット商品単位
			}
			
			//ロケーションマスタ
			Object[][] LocationMstRt = LocationMstRt(GetClCd,GetWhCd,GetLoc);
			if(1==LocationMstRt.length) {
				GetLoc				= (String)LocationMstRt[0][M100_LocationMstRt.ColLoc];			//ロケーション
				GetLocName			= (String)LocationMstRt[0][M100_LocationMstRt.ColLocName];		//ロケーション名
				GetType				= (int)LocationMstRt[0][M100_LocationMstRt.ColType];			//ロケタイプ
			}
		}
		
		//入荷実績日空白で、入荷日管理しない場合デフォルト入荷実績日セット
		if(B100_DefaultVariable.ActualDateUnControl && "".equals(GetActualDate)) {
			GetActualDate = B100_DefaultVariable.DefaultActualDate;
		}
		//賞味期限空白の場合、一旦デフォルト賞味期限セット
		if("".equals(GetExpdate)) {
			GetExpdate = B100_DefaultVariable.DefaultExpDate;
		}
		...
		******************************************/
		...
		final JFrame main_fm 	= B100_FrameParts.FrameCreate(x,y,800,700,"Corgi00在庫調整","ZK");
		...
		final JFormattedTextField TB_CtUnitQty		= B100_FrameParts.JFormattedTextFieldSet(400,250,70,20,""	+ni.format(GetCtUnitQty)		,11,1,"#,###");
		final JFormattedTextField TB_CsUnitQty		= B100_FrameParts.JFormattedTextFieldSet(400,275,70,20,""	+ni.format(GetCsUnitQty)		,11,1,"#,###");
		final JFormattedTextField TB_PlUnitQty		= B100_FrameParts.JFormattedTextFieldSet(400,300,70,20,""	+ni.format(GetPlUnitQty)		,11,1,"#,###");
		
		JLabel LB_Qty				= B100_FrameParts.JLabelSet(	  0,450,150,20,"調整前数量:"		,11,1);
		JLabel LB_ShipPlanQty		= B100_FrameParts.JLabelSet(	  0,475,150,20,"調整前引当済数:"	,11,1);
		JLabel LB_PossibleQty		= B100_FrameParts.JLabelSet(	  0,500,150,20,"調整前出荷可能数:"	,11,1);
		
		JLabel LB_Msg				= B100_FrameParts.JLabelSet(	270,475, 70,20,"⇒⇒⇒"				,11,2);
		...
		JLabel LB_PlAdjustUnitName	= B100_FrameParts.JLabelSet(	300,350,100,20,GetPlUnitName		,11,0);
		JLabel LB_CsAdjustUnitName	= B100_FrameParts.JLabelSet(	300,375,100,20,GetCsUnitName		,11,0);
		JLabel LB_CtAdjustUnitName	= B100_FrameParts.JLabelSet(	300,400,100,20,GetCtUnitName		,11,0);
		JLabel LB_BrAdjustUnitName	= B100_FrameParts.JLabelSet(	300,425,100,20,GetUnitName			,11,0);
		
		
		JLabel LB_AfterQty			= B100_FrameParts.JLabelSet(	320,450,150,20,"調整後数量:"		,11,1);
		JLabel LB_AfterShipPlanQty	= B100_FrameParts.JLabelSet(	320,475,150,20,"調整後引当済数:"	,11,1);
		JLabel LB_AfterPossibleQty	= B100_FrameParts.JLabelSet(	320,500,150,20,"調整後出荷可能数:"	,11,1);
		...
		final JTextField TB_Loc		= B100_FrameParts.JTextFieldSet(	150,100,200,20,GetLoc,11,0);							//ロケーション
		final JTextField TB_LocName	= B100_FrameParts.JTextFieldSet(	150,125,200,20,GetLocName,11,0);						//ロケーション名
		final JComboBox TB_LocType	= B100_FrameParts.JComboBoxSet(	150,150,200,20,B100_DefaultVariable.LocType[0],11);	//ロケータイプ
		final JTextField TB_ItemCd	= B100_FrameParts.JTextFieldSet(	150,175,200,20,GetItemCd,11,0);							//商品コード
		final JTextField TB_ItemName= B100_FrameParts.JTextFieldSet(	150,200,200,20,GetItemName01,11,0);						//商品名
		final JTextField TB_Lot		= B100_FrameParts.JTextFieldSet(	150,225,200,20,GetLot,11,0);							//ロット
		final JFormattedTextField TB_Expdate			= B100_FrameParts.JFormattedTextFieldSet(	150,250, 70,20,GetExpdate,11,0,"YYYY/MM/DD");				//消費期限
		final JFormattedTextField TB_ActualDate			= B100_FrameParts.JFormattedTextFieldSet(	150,275, 70,20,GetActualDate,11,0,"YYYY/MM/DD");			//入荷実績日
		
		final JFormattedTextField TB_Qty				= B100_FrameParts.JFormattedTextFieldSet(	150,450, 70,20,""+ni.format(GetQty),11,1,"#,###");			//調整前数量
		final JFormattedTextField TB_ShipPlanQty		= B100_FrameParts.JFormattedTextFieldSet(	150,475, 70,20,""+ni.format(GetShipPlanQty),11,1,"#,###");	//調整前引当済数
		final JFormattedTextField TB_PossibleQty		= B100_FrameParts.JFormattedTextFieldSet(	150,500, 70,20,""+ni.format(GetPossibleQty),11,1,"#,###");	
		...
		final JFormattedTextField TB_AfterQty			= B100_FrameParts.JFormattedTextFieldSet(	470,450, 70,20,""+ni.format(GetQty)			,11,1,"#,###");				//調整後数量
		final JFormattedTextField TB_AfterShipPlanQty	= B100_FrameParts.JFormattedTextFieldSet(	470,475, 70,20,""+ni.format(GetShipPlanQty)	,11,1,"#,###");				//調整後引当済数
		final JFormattedTextField TB_AfterPossibleQty	= B100_FrameParts.JFormattedTextFieldSet(	470,500, 70,20,""+ni.format(GetPossibleQty)	,11,1,"#,###");				//調整後出荷可能数
		
		TB_ClCd.setSelectedIndex(	GetSelectIndex(B100_DefaultVariable.ClList[1]		,GetClCd ) );		//荷主コード
		TB_WhCd.setSelectedIndex(	GetSelectIndex(B100_DefaultVariable.ClList[1]		,GetWhCd ) );		//倉庫コード
		TB_LocType.setSelectedIndex(GetSelectIndex(B100_DefaultVariable.LocType[1]		,GetType+"" ) );	//ロケタイプ
			...
			public void focusLost(FocusEvent e){
				if(RenewFg) {
					...
					if(!"".equals(SetClCd)
						...
						if(1==StockRt.length) {
							TB_LocName.setText((String)StockRt[0][T100_StockRt.ColLocName]);										//ロケーション名
							TB_ItemName.setText((String)StockRt[0][T100_StockRt.ColItemName01]);									//商品名
							...
							for(int i=0;i<B100_DefaultVariable.ShipPlovisionUnTgtList.length;i++) {
								if(B100_DefaultVariable.ShipPlovisionUnTgtList[i].equals(""+GetType)) {
									ShipTgtLoc = false;
								}
							}
							if(ShipTgtLoc) {
		...
		TB_AdjustQty.addFocusListener(new FocusAdapter(){
			...
			public void focusLost(FocusEvent e){
				if(RenewFg) {
					...
					int LocType	= TextToInt(B100_DefaultVariable.LocType[1][TB_LocType.getSelectedIndex()]);	//ロケータイプ
