...
public class M100_WhMstRt{
	...
	String GetNoTel			= (String)WhMstRt[i][M100_WhMstRt.ColNoTel];			//拠点倉庫電話
	String GetNoFax			= (String)WhMstRt[i][M100_WhMstRt.ColNoFax];			//拠点倉庫FAX
	String GetNoMail		= (String)WhMstRt[i][M100_WhMstRt.ColNoMail];			//拠点倉庫MAIL
	...
	*/
	...
	public static Object[][] WhMstRt(
		...
		SearchWHName	= B100_ArrayListControl.ArryListStringUniqueList(SearchWHName);
		...
		String sql = " select "
				+"("+A00000_Main.MySqlDefaultSchemaNYANKO+".KM0010_WHMST.WHCD) as WHCD,\n"				//倉庫コード
				+"("+A00000_Main.MySqlDefaultSchemaNYANKO+".KM0010_WHMST.WHName) as WHName,\n"			//拠点倉庫名
				+"("+A00000_Main.MySqlDefaultSchemaNYANKO+".KM0010_WHMST.Post) as Post,\n"				//拠点倉庫郵便番号
				+"("+A00000_Main.MySqlDefaultSchemaNYANKO+".KM0010_WHMST.Add01) as Add01,\n"				//拠点倉庫住所1
				+"("+A00000_Main.MySqlDefaultSchemaNYANKO+".KM0010_WHMST.Add02) as Add02,\n"				//拠点倉庫住所2
				+"("+A00000_Main.MySqlDefaultSchemaNYANKO+".KM0010_WHMST.Tel) as Tel,\n"					//拠点倉庫電話
				+"("+A00000_Main.MySqlDefaultSchemaNYANKO+".KM0010_WHMST.Fax) as Fax,\n"					//拠点倉庫FAX
				+"("+A00000_Main.MySqlDefaultSchemaNYANKO+".KM0010_WHMST.Mail) as Mail,\n"				//拠点倉庫MAIL
				+"("+A00000_Main.MySqlDefaultSchemaNYANKO+".KM0010_WHMST.Com01) as Com01,\n"				//コメント１
				+"("+A00000_Main.MySqlDefaultSchemaNYANKO+".KM0010_WHMST.Com02) as Com02,\n"				//コメント２
				+"("+A00000_Main.MySqlDefaultSchemaNYANKO+".KM0010_WHMST.Com03) as Com03,\n"				//コメント３
				+"("+A00000_Main.MySqlDefaultSchemaNYANKO+".KM0010_WHMST.PTMSCD) as PTMSCD,\n"			//基幹システム連携用事業所CD
				+"("+A00000_Main.MySqlDefaultSchemaNYANKO+".KM0010_WHMST.EntryDate) as EntryDate,\n"		//登録者コード
				+"("+A00000_Main.MySqlDefaultSchemaNYANKO+".KM0010_WHMST.UpdateDate) as UpdateDate,\n"	//更新者コード
				+"("+A00000_Main.MySqlDefaultSchemaNYANKO+".KM0010_WHMST.EntryUser) as EntryUser,\n"		//データ登録日時
				+"("+A00000_Main.MySqlDefaultSchemaNYANKO+".KM0010_WHMST.UpdateUser) as UpdateUser\n"	//データ更新日時
				+ " from "+A00000_Main.MySqlDefaultSchemaNYANKO+".KM0010_WHMST"
				+ " where 1=1";
		...
		if(null!=SearchWHCD&&0<SearchWHCD.size()){
			...
			for(int i=0;i<SearchWHCD.size();i++){
				if(0<i){sql = sql + " or ";}
				sql = sql + " "+A00000_Main.MySqlDefaultSchemaNYANKO+".KM0010_WHMST.WHCD = ?";
			}
			sql = sql + ")";
		}
		if(null!=SearchWHName&&0<SearchWHName.size()){
			...
			for(int i=0;i<SearchWHName.size();i++){
				if(0<i){sql = sql + " or ";}
				sql = sql + " "+A00000_Main.MySqlDefaultSchemaNYANKO+".KM0010_WHMST.WHName like ?";
			}
			sql = sql + ")";
		}
		if(null!=SearchPost&&0<SearchPost.size()){
			...
			for(int i=0;i<SearchPost.size();i++){
				if(0<i){sql = sql + " or ";}
				sql = sql + " "+A00000_Main.MySqlDefaultSchemaNYANKO+".KM0010_WHMST.Post like ?";
			}
			sql = sql + ")";
		}
		if(null!=SearchAdd&&0<SearchAdd.size()){
			...
			for(int i=0;i<SearchAdd.size();i++){
				if(0<i){sql = sql + " or ";}
				sql = sql + " CONCAT ("+A00000_Main.MySqlDefaultSchemaNYANKO+".KM0010_WHMST.Add01";
				sql = sql + " , "+A00000_Main.MySqlDefaultSchemaNYANKO+".KM0010_WHMST.Add02) like ?";
			}
			sql = sql + ")";
		}
		if(null!=SearchTel&&0<SearchTel.size()){
			...
			for(int i=0;i<SearchTel.size();i++){
				if(0<i){sql = sql + " or ";}
				sql = sql + " "+A00000_Main.MySqlDefaultSchemaNYANKO+".KM0010_WHMST.Tel like ?";
			}
			sql = sql + ")";
		}
		if(null!=SearchFax&&0<SearchFax.size()){
			...
			for(int i=0;i<SearchFax.size();i++){
				if(0<i){sql = sql + " or ";}
				sql = sql + " "+A00000_Main.MySqlDefaultSchemaNYANKO+".KM0010_WHMST.Fax like ?";
			}
			sql = sql + ")";
		}
		if(null!=SearchMail&&0<SearchMail.size()){
			...
			for(int i=0;i<SearchMail.size();i++){
				if(0<i){sql = sql + " or ";}
				sql = sql + " "+A00000_Main.MySqlDefaultSchemaNYANKO+".KM0010_WHMST.Mail like ?";
			}
			sql = sql + ")";
		}
		...
		if(null!=SearchCom&&0<SearchCom.size()){
			...
			for(int i=0;i<SearchCom.size();i++){
				if(0<i){sql = sql + " or ";}
				sql = sql + " "+A00000_Main.MySqlDefaultSchemaNYANKO+".KM0010_WHMST.Com01 like ?";
				sql = sql + " or "+A00000_Main.MySqlDefaultSchemaNYANKO+".KM0010_WHMST.Com02 like ?";
				sql = sql + " or "+A00000_Main.MySqlDefaultSchemaNYANKO+".KM0010_WHMST.Com03 like ?";
			}
			sql = sql + ")";
		}
		if(null!=SearchPTMSCD&&0<SearchPTMSCD.size()){
			...
			for(int i=0;i<SearchPTMSCD.size();i++){
				if(0<i){sql = sql + " or ";}
				sql = sql + " "+A00000_Main.MySqlDefaultSchemaNYANKO+".KM0010_WHMST.PTMSCD = ?";
			}
			sql = sql + ")";
		}
		
		sql = sql + " order by "+A00000_Main.MySqlDefaultSchemaNYANKO+".KM0010_WHMST.WHCD";
		...
		if(SearchKick) {
			...
			try {
				...
				while (rset01.next()) {
					...
					if(null==rset01.getTimestamp("EntryDate")){	rt[counter][ColNoEntryDate]	="";}else{rt[counter][ColNoEntryDate]		=B100_DateTimeControl.dtmString2(rset01.getTimestamp("EntryDate"))[1];}			//データ登録日時
					if(null==rset01.getTimestamp("UpdateDate")){rt[counter][ColNoUpdateDate]	="";}else{rt[counter][ColNoUpdateDate]	=B100_DateTimeControl.dtmString2(rset01.getTimestamp("UpdateDate"))[1];}		//データ更新日時
	...
	public static String[] NewWhCdGet(int NeedCount) {
    	...
    	for(int i=0;i<WhMstRt.length;i++) {
    		if(4<(""+WhMstRt[i][M100_WhMstRt.ColNoWHCD]).length()&&"ATWH".equals((""+WhMstRt[i][M100_WhMstRt.ColNoWHCD]).substring(0,4))) {
    			String WST = B100_TextControl.num_only_String(""+WhMstRt[i][M100_WhMstRt.ColNoWHCD]);
    			if("".equals(WST)){WST = "0";}
