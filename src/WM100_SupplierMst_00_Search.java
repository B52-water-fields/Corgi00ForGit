...
public class WM100_SupplierMst_00_Search{
	static int SetX;
	static int SetY;
	...
	public static void SupplierMstSearch(int x,int y) {
		...
		final JFrame main_fm = B100_FrameParts.FrameCreate(x,y,900,750,"Corgi00仕入先マスタ検索　WM100_SupplierMst_00_Search","");
		...
		//検索条件パネル
		JPanel PN_Search = B100_FrameParts.JPanelSet(10,40,880,300,"White");
		...
		TB_SearchPayDateEnd.setSelectedIndex(B100_DefaultVariable.ShimeDateList[0].length-1);
		TB_SearchShimeDateStr.setSelectedIndex(0);
		TB_SearchShimeDateEnd.setSelectedIndex(B100_DefaultVariable.ShimeDateList[0].length-1);
		...
		for(int i=0;i<RtSupplierRt.length;i++) {
			columnNames01[1+(int)RtSupplierRt[i][1]] = ""+RtSupplierRt[i][3];
		}
		...
		for(int i=0;i<RtSupplierRt.length;i++) {
			if("int".equals((String)RtSupplierRt[i][2])||"float".equals((String)RtSupplierRt[i][2])) {
				column = columnModel01.getColumn(1+(int)RtSupplierRt[i][1]);	column.setPreferredWidth( 90*A00000_Main.Mul/A00000_Main.Div);	column.setCellRenderer(B100_FrameParts.rightCellRenderer());
			}else {
				column = columnModel01.getColumn(1+(int)RtSupplierRt[i][1]);	column.setPreferredWidth( 90*A00000_Main.Mul/A00000_Main.Div);	column.setCellRenderer(B100_FrameParts.leftCellRenderer());
			}
		}
		//スクロール用設定
		JScrollPane scpn01 = B100_FrameParts.JScrollPaneSet(10,350,870,275,tb01);
		main_fm.add(scpn01);
		...
		SearchBtn.addActionListener(new AbstractAction(){
			public void actionPerformed(ActionEvent e){
				if(RenewFg) {
					...
					String GetSearchClWh			= B100_DefaultVariable.SearchWhList[1][TB_SearchClWh.getSelectedIndex()];		//担当倉庫
					...
					String GetSearchPayDateStr 		= B100_DefaultVariable.ShimeDateList[1][TB_SearchPayDateStr.getSelectedIndex()];		//支払日（日＝99）開始
					String GetSearchPayDateEnd 		= B100_DefaultVariable.ShimeDateList[1][TB_SearchPayDateEnd.getSelectedIndex()];		//支払日（日＝99）終了
					String GetSearchShimeDateStr 	= B100_DefaultVariable.ShimeDateList[1][TB_SearchShimeDateStr.getSelectedIndex()];	//締め日（末日＝99）開始
					String GetSearchShimeDateEnd 	= B100_DefaultVariable.ShimeDateList[1][TB_SearchShimeDateEnd.getSelectedIndex()];	//締め日（末日＝99）終了
					...
					GetSearchClWh			= B100_TextControl.Trim(GetSearchClWh);			//担当倉庫
					GetSearchClCd			= B100_TextControl.Trim(GetSearchClCd);			//荷主CD
					GetSearchSPCd 			= B100_TextControl.Trim(GetSearchSPCd);			//仕入先コード
					GetSearchSPName 		= B100_TextControl.Trim(GetSearchSPName);			//仕入先名
					GetSearchSPPost 		= B100_TextControl.Trim(GetSearchSPPost);			//仕入先郵便
					GetSearchSPAdd 			= B100_TextControl.Trim(GetSearchSPAdd);			//仕入先住所
					GetSearchSPTel 			= B100_TextControl.Trim(GetSearchSPTel);			//仕入先電話
					GetSearchSPFax 			= B100_TextControl.Trim(GetSearchSPFax);			//仕入先FAX
					GetSearchSPMail 		= B100_TextControl.Trim(GetSearchSPMail);			//仕入先MAIL
					GetSearchCom 			= B100_TextControl.Trim(GetSearchCom);			//コメント
					GetSearchPTMSCDBMN 		= B100_TextControl.Trim(GetSearchPTMSCDBMN);		//基幹Sysコード（部門）
					GetSearchPTMSCDNINUSHI 	= B100_TextControl.Trim(GetSearchPTMSCDNINUSHI);	//基幹Sysコード（荷主）
					GetSearchPaySiteStr 	= B100_TextControl.Trim(GetSearchPaySiteStr);		//支払いサイト（月数）開始
					GetSearchPaySiteEnd		= B100_TextControl.Trim(GetSearchPaySiteEnd);		//支払いサイト（月数）終了
					GetSearchPayDateStr 	= B100_TextControl.Trim(GetSearchPayDateStr);		//支払日（日＝99）開始
					GetSearchPayDateEnd 	= B100_TextControl.Trim(GetSearchPayDateEnd);		//支払日（日＝99）終了
					GetSearchShimeDateStr 	= B100_TextControl.Trim(GetSearchShimeDateStr);	//締め日（末日＝99）開始
					GetSearchShimeDateEnd 	= B100_TextControl.Trim(GetSearchShimeDateEnd);	//締め日（末日＝99）終了
					GetSearchDECD 			= B100_TextControl.Trim(GetSearchDECD);			//納品先コード
					GetSearchDepartmentCd 	= B100_TextControl.Trim(GetSearchDepartmentCd);	//部署CD

					GetSearchPaySiteStr 	= B100_TextControl.num_only_String02(GetSearchPaySiteStr);		//支払いサイト（月数）開始
					GetSearchPaySiteEnd		= B100_TextControl.num_only_String02(GetSearchPaySiteEnd);		//支払いサイト（月数）終了
					GetSearchPayDateStr 	= B100_TextControl.num_only_String02(GetSearchPayDateStr);		//支払日（日＝99）開始
					GetSearchPayDateEnd 	= B100_TextControl.num_only_String02(GetSearchPayDateEnd);		//支払日（日＝99）終了
					GetSearchShimeDateStr 	= B100_TextControl.num_only_String02(GetSearchShimeDateStr);	//締め日（末日＝99）開始
					GetSearchShimeDateEnd 	= B100_TextControl.num_only_String02(GetSearchShimeDateEnd);	//締め日（末日＝99）終了
				...
				if(RenewFg) {
					...
					for(int i=0;i<RowCount;i++) {
						if((boolean)MainFmTableModel.getValueAt(i, 0)) {
							TgtClCd = ""+MainFmTableModel.getValueAt(i, M100_SupplierRt.ColClCd+1);
							TgtWhCd = ""+MainFmTableModel.getValueAt(i, M100_SupplierRt.ColClWh+1);
							TgtSupplierCd 	= ""+MainFmTableModel.getValueAt(i, M100_SupplierRt.ColSPCd+1);
							KickFg = true;
						}
					}
					if(KickFg) {
						...
						main_fm.dispose();
						WM100_SupplierMst_01_RenewAndCreate.SupplierMstRenewAndCreate(0,0,TgtClCd,TgtWhCd,TgtSupplierCd);
					}
					RenewFg = true;
				}
			}
			...
			public void actionPerformed(ActionEvent e){
				if(RenewFg) {
					...
					if(null!=Selected && !Selected.equals(Selected.replace(".xlsx", ""))) {
						...
						main_fm.setVisible(false);
						main_fm.dispose();
						WM100_SupplierMst_02_ExcelEntry.SupplierMstExcelEntry(0,0,Selected);
					}
					RenewFg = true;
				}
			}
		...
		CsvBtn.addActionListener(new AbstractAction(){
			public void actionPerformed(ActionEvent e){
				if(RenewFg) {
					RenewFg = false;
					B100_TableControl.TableOutPutCsv("出力先選択","仕入先マスタ検索結果",tb01);
					RenewFg = true;
				}
			}
		});
		...
		ExcelBtn.addActionListener(new AbstractAction(){
			public void actionPerformed(ActionEvent e){
				if(RenewFg) {
					RenewFg = false;
					B100_TableControl.TableOutPutExcel("出力先選択","仕入先マスタ検索結果",tb01);
					RenewFg = true;
				}
			}
		});
