...
public class WM100_ItemRecomendLocMst_02_ExcelEntry{
	static int SetX;
	static int SetY;
	...
	public static void ItemRecomendLocMstExcelEntryMain(int x,int y,String TgtFilePath,String SheetName) {
		...
		JScrollPane scpn01 = B100_FrameParts.JScrollPaneSet(10,65,460,600,tb01);
		main_fm.add(scpn01);
				...
				for(int i01=0;i01<((String[])SetObRt[i][4]).length;i01++) {
					if("".equals((String)((String[])SetObRt[i][4])[i01])) {
						...
						for(int i02=0;i02<ItemMstRt.length;i02++) {
							String MstItemCd = (String)ItemMstRt[i02][M100_ItemMstRt.ColItemCd];				//商品コード
				...
				for(int i01=0;i01<((String[])SetObRt[i][4]).length;i01++) {
					if("".equals((String)((String[])SetObRt[i][4])[i01])) {
						...
						for(int i02=0;i02<LocMstRt.length;i02++) {
							String MstLoc	= (String)LocMstRt[i02][M100_LocationMstRt.ColLoc];			//ロケーション
