...
public class WM100_CarMst_00_Search{
	static int SetX;
	static int SetY;
	...
	public static void CarMstSearch(int x,int y) {
		...
		final JFrame main_fm = B100_FrameParts.FrameCreate(x,y,860,750,"Corgi00車輛検索　WM100_CarMst_00_Search","");
		...
		final JTextField TB_SearchCarCd				= B100_FrameParts.JTextFieldSet(100, 75,100,20,"",11,0);	//車輛CD
		final JTextField TB_SearchCarName			= B100_FrameParts.JTextFieldSet(100,100,200,20,"",11,0);	//車輛名
		final JComboBox  TB_SearchDelFg				= B100_FrameParts.JComboBoxSet( 100,125,100,20,B100_DefaultVariable.SearchDelList[0],11);	//削除区分
		...
		for(int i=0;i<RtSettingCarMstRt.length;i++) {
			columnNames01[1+(int)RtSettingCarMstRt[i][1]] = ""+RtSettingCarMstRt[i][3];
		}
		...
		for(int i=0;i<RtSettingCarMstRt.length;i++) {
			if("int".equals((String)RtSettingCarMstRt[i][2])||"float".equals((String)RtSettingCarMstRt[i][2])) {
				column = columnModel01.getColumn(1+(int)RtSettingCarMstRt[i][1]);	column.setPreferredWidth( 90*A00000_Main.Mul/A00000_Main.Div);	column.setCellRenderer(B100_FrameParts.rightCellRenderer());
			}else {
				column = columnModel01.getColumn(1+(int)RtSettingCarMstRt[i][1]);	column.setPreferredWidth( 90*A00000_Main.Mul/A00000_Main.Div);	column.setCellRenderer(B100_FrameParts.leftCellRenderer());
			}
		}
		
		//スクロール用設定
		JScrollPane scpn01 = B100_FrameParts.JScrollPaneSet(10,210,820,395,tb01);
		main_fm.add(scpn01);
		...
		SearchBtn.addActionListener(new AbstractAction(){
			public void actionPerformed(ActionEvent e){
				if(RenewFg) {
					...
					GetSearchCarCd	 			= B100_TextControl.Trim(GetSearchCarCd);
					GetSearchCarName	 		= B100_TextControl.Trim(GetSearchCarName);
				...
				if(RenewFg) {
					...
					for(int i=0;i<RowCount;i++) {
						if((boolean)MainFmTableModel.getValueAt(i, 0)) {
							TgtWhCd 				= ""+MainFmTableModel.getValueAt(i, M100_CarMstRt.ColWHCD+1);					if(null==TgtWhCd				) {TgtWhCd="";}
							TgtShippingCompanyCd 	= ""+MainFmTableModel.getValueAt(i, M100_CarMstRt.ColShippingCompanyCd+1);	if(null==TgtShippingCompanyCd	) {TgtShippingCompanyCd="";}
							TgtCar 					= ""+MainFmTableModel.getValueAt(i, M100_CarMstRt.ColCarCd+1);					if(null==TgtCar					) {TgtCar="";}
						}
					}
					if(!"".equals(TgtCar)) {
						SetX=main_fm.getX();
						...
						main_fm.dispose();
						WM100_CarMst_01_RenewAndCreate.CarMstRenewAndCreate(0,0,TgtWhCd,TgtShippingCompanyCd,TgtCar);
					}
					RenewFg = true;
				}
			}
		...
		CsvBtn.addActionListener(new AbstractAction(){
			public void actionPerformed(ActionEvent e){
				if(RenewFg) {
					RenewFg = false;
					B100_TableControl.TableOutPutCsv("出力先選択","車輛マスタ検索結果",tb01);
					RenewFg = true;
				}
			}
		});
		...
		ExcelBtn.addActionListener(new AbstractAction(){
			public void actionPerformed(ActionEvent e){
				if(RenewFg) {
					RenewFg = false;
					B100_TableControl.TableOutPutExcel("出力先選択","車輛マスタ検索結果",tb01);
					RenewFg = true;
				}
			}
		});
			...
			public void actionPerformed(ActionEvent e){
				if(RenewFg) {
					...
					if(null!=Selected && !Selected.equals(Selected.replace(".xlsx", ""))) {
						...
						main_fm.setVisible(false);
						main_fm.dispose();
						WM100_CarMst_02_ExcelEntry.CarMstExcelEntry(0,0,Selected);
					}
					RenewFg = true;
				}
			}
