...
public class WM100_LocationMst_02_ExcelEntry{
	...
	public static void LocationMstExcelEntry(int x,int y,String TgtFilePath) {
		...
		final JFrame main_fm = B100_FrameParts.FrameCreate(x,y,600,200,"Corgi00ロケーションマスタ登録（エクセル）　WM100_LocationMst_02_ExcelEntry","");
	...
	public static void LocationMstExcelEntryMain(int x,int y,String TgtFilePath,String SheetName) {
		...
		if(y==0) {y=SetY;}
		RenewFg = false;
		final JFrame main_fm = B100_FrameParts.FrameCreate(x,y,750,800,"Corgi00ロケーションマスタ登録（エクセル）　WM100_LocationMst_02_ExcelEntry","");
		JLabel userinfo = B100_FrameParts.UserInfo();
		JButton exit_btn = B100_FrameParts.ExitBtn();
		JButton entry_btn = B100_FrameParts.EntryBtn();
