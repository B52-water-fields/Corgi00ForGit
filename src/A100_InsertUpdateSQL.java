...
public class A100_InsertUpdateSQL{
	//
	//データの登録更新をまとめて行います
	// ==========================================================================
    //  A100_InsertUpdateSQL（ウリエル／判断と裁定の天使　データベースの園の門番）
    // ==========================================================================
    //
		...
		if(
			...
			for(int i01=0;i01<field_name.length;i01++){
				for(int i02=0;i02<field_name[i01].length;i02++){
					...
					field_name[i01][i02] = B100_TextControl.Trim(field_name[i01][i02]);
				}
			}
			for(int i01=0;i01<entry_data.length;i01++){
				for(int i02=0;i02<entry_data[i01].length;i02++){
					...
					entry_data[i01][i02] = B100_TextControl.Trim(entry_data[i01][i02]);
				}
			}
			for(int i01=0;i01<judg_field.length;i01++){
					if(null==judg_field[i01]){judg_field[i01]="";}
					judg_field[i01] = B100_TextControl.Trim(judg_field[i01]);
			}
			for(int i01=0;i01<judg_data.length;i01++){
				for(int i02=0;i02<judg_data[i01].length;i02++){
					...
					judg_data[i01][i02] = B100_TextControl.Trim(judg_data[i01][i02]);
				}
			}
			//現在の日付時刻の取得
			...
			try {
				...
				sql = sql +")";
				//System.out.println(sql);
				stmt02 = A100_DbConnect.conn.prepareStatement(sql);
				...
				//System.out.println(sql);
				stmt03 = A100_DbConnect.conn.prepareStatement(sql);
