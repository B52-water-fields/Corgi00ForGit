...
public class WM100_ItemComversionMst_00_Search{
	static int SetX;
	static int SetY;
	...
	public static void ItemComversionMstSearch(int x,int y) {
		...
		final JFrame main_fm = B100_FrameParts.FrameCreate(x,y,1300,750,"Corgi00商品変換マスタ検索","");
		...
		//検索ボタン
		JButton SearchBtn = B100_FrameParts.BtnSet(130,175,100,20,"検索",11);
		...
		for(int i=0;i<RtItemComversionMstRt.length;i++) {
			columnNames01[1+(int)RtItemComversionMstRt[i][1]] = ""+RtItemComversionMstRt[i][3];
		}
		...
		for(int i=0;i<RtItemComversionMstRt.length;i++) {
			if("int".equals((String)RtItemComversionMstRt[i][2])||"float".equals((String)RtItemComversionMstRt[i][2])) {
				column = columnModel01.getColumn(1+(int)RtItemComversionMstRt[i][1]);	column.setPreferredWidth( 90*A00000_Main.Mul/A00000_Main.Div);	column.setCellRenderer(B100_FrameParts.rightCellRenderer());
			}else {
				column = columnModel01.getColumn(1+(int)RtItemComversionMstRt[i][1]);	column.setPreferredWidth( 90*A00000_Main.Mul/A00000_Main.Div);	column.setCellRenderer(B100_FrameParts.leftCellRenderer());
			}
		}
				...
				if(RenewFg) {
					...
					for(int i=0;i<RowCount;i++) {
						if((boolean)tableModel_ms01.getValueAt(i, 0)) {
							TgtClgpCd 	= ""+tableModel_ms01.getValueAt(i,M100_ItemComversionMstRt.ColClGpCd+1);
							TgtClCd		= ""+tableModel_ms01.getValueAt(i,M100_ItemComversionMstRt.ColClCd+1);
							TgtClItemCd 	= ""+tableModel_ms01.getValueAt(i,M100_ItemComversionMstRt.ColClItemCd+1);
						}
					}
					if(!"".equals(TgtClgpCd) && !"".equals(TgtClCd) && !"".equals(TgtClItemCd)) {
						...
						WM100_ItemComversionMst_01_RenewAndCreate.ItemComversionMstRenewAndCreate(0,0,TgtClgpCd,TgtClCd,TgtClItemCd);
					}
					RenewFg = true;
				}
			}
		...
		CreateBtn.addActionListener(new AbstractAction(){
			public void actionPerformed(ActionEvent e){
				if(RenewFg) {
					...
					WM100_ItemComversionMst_01_RenewAndCreate.ItemComversionMstRenewAndCreate(0,0,"","","");
					RenewFg = true;
				}
			}
		});
		...
		CsvBtn.addActionListener(new AbstractAction(){
			public void actionPerformed(ActionEvent e){
				if(RenewFg) {
					RenewFg = false;
					B100_TableControl.TableOutPutCsv("出力先選択","商品変換マスタ検索結果",tb01);
					RenewFg = true;
				}
			}
		});
		...
		ExcelBtn.addActionListener(new AbstractAction(){
			public void actionPerformed(ActionEvent e){
				if(RenewFg) {
					RenewFg = false;
					B100_TableControl.TableOutPutExcel("出力先選択","商品変換マスタ検索結果",tb01);
					RenewFg = true;
				}
			}
		});
			...
			public void actionPerformed(ActionEvent e){
				if(RenewFg) {
					...
					if(null!=Selected && !Selected.equals(Selected.replace(".xlsx", ""))) {
						...
						main_fm.setVisible(false);
						main_fm.dispose();
						WM100_ItemComversionMst_02_ExcelEntry.ItemComversionMstExcelEntry(0,0,Selected);
					}
					RenewFg = true;
				}
			}
