...
public class WM100ItemComversionMstSearch{
	static int SetX;
	static int SetY;
	...
	public static void ItemComversionMstSearch(int x,int y) {
		...
		final JFrame main_fm = B100FrameParts.FrameCreate(x,y,1300,750,"Corgi00商品変換マスタ検索","");
		...
		JLabel LB_SearchItemCd		= B100FrameParts.JLabelSet(	  0, 75,130,20,"商品コード:"			,11,1);
		JLabel LB_SearchCLItemCd	= B100FrameParts.JLabelSet(	  0,100,130,20,"荷主商品コード:"		,11,1);
		JLabel LB_SearchItemName	= B100FrameParts.JLabelSet(	  0,125,130,20,"商品名:"				,11,1);
		
		final JComboBox   TB_SearchClGpCd	= B100FrameParts.JComboBoxSet(	130, 25,250,20,B100DefaultVariable.SearchClGpList[0],11);	//荷主グループコード
		final JComboBox   TB_SearchClCd		= B100FrameParts.JComboBoxSet(	130, 50,250,20,B100DefaultVariable.SearchClList[0],11);		//荷主コード
		final JTextField  TB_SearchItemCd	= B100FrameParts.JTextFieldSet(	130, 75,100,20,""	,11,0);		//商品コード
		final JTextField  TB_SearchCLItemCd	= B100FrameParts.JTextFieldSet(	130,100,100,20,""	,11,0);		//荷主商品コード
		final JTextField  TB_SearchItemName	= B100FrameParts.JTextFieldSet(	130,125,100,20,""	,11,0);		//商品名
		
		JLabel LB2_SearchItemCd		= B100FrameParts.JLabelSet(	230, 75, 80,20,"と一致"	,11,0);
		JLabel LB2_SearchCLItemCd	= B100FrameParts.JLabelSet(	230,100, 80,20,"と一致"	,11,0);
		JLabel LB2_SearchItemName	= B100FrameParts.JLabelSet(	230,125, 80,20,"を含む"	,11,0);
		...
		//検索ボタン
		JButton SearchBtn = B100FrameParts.BtnSet(130,175,100,20,"検索",11);
		...
		for(int i=0;i<RtItemComversionMstRt.length;i++) {
			if("int".equals((String)RtItemComversionMstRt[i][2])||"float".equals((String)RtItemComversionMstRt[i][2])) {
		...
		SearchBtn.addActionListener(new AbstractAction(){
			public void actionPerformed(ActionEvent e){
				if(RenewFg) {
					...
					GetSearchItemName	= B100TextControl.Trim(GetSearchCLItemCd);	//商品名
			...
			public void actionPerformed(ActionEvent e){
				if(RenewFg) {
					...
					if(!"".equals(TgtClgpCd) && !"".equals(TgtClCd) && !"".equals(TgtClItemCd)) {
						...
						WM100ItemComversionMstRenewAndCreate.ItemComversionMstRenewAndCreate(0,0,TgtClgpCd,TgtClCd,TgtClItemCd);
					}
					RenewFg = true;
				}
			}
		...
		CreateBtn.addActionListener(new AbstractAction(){
			public void actionPerformed(ActionEvent e){
				if(RenewFg) {
					...
					WM100ItemComversionMstRenewAndCreate.ItemComversionMstRenewAndCreate(0,0,"","","");
					RenewFg = true;
				}
			}
		});
		...
		CsvBtn.addActionListener(new AbstractAction(){
			public void actionPerformed(ActionEvent e){
				if(RenewFg) {
					RenewFg = false;
					B100TableControl.TableOutPutCsv("出力先選択","商品変換マスタ検索結果",tb01);
					RenewFg = true;
				}
			}
		});
		...
		ExcelBtn.addActionListener(new AbstractAction(){
			public void actionPerformed(ActionEvent e){
				if(RenewFg) {
					RenewFg = false;
					B100TableControl.TableOutPutExcel("出力先選択","商品変換マスタ検索結果",tb01);
					RenewFg = true;
				}
			}
		});
			...
			public void actionPerformed(ActionEvent e){
				if(RenewFg) {
					...
					if(null!=Selected && !Selected.equals(Selected.replace(".xlsx", ""))) {
						...
						main_fm.setVisible(false);
						main_fm.dispose();
						WM100ItemComversionMstExcelEntry.ItemComversionMstExcelEntry(0,0,Selected);
					}
					RenewFg = true;
				}
			}
