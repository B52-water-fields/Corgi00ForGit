...
public class WT100ArrivalPlanArrayEntrySourceDataView{
	...
	public static void ArrivalPlanArrayEntrySourceDataView(int x,int y,String[][] EntryData) {
		...
		Object[][] RtArrivalPlanArrayEntry = WT100ArrivalPlanArrayEntrySourceDataView.RtArrivalPlanArrayEntrySourceDataView();
		...
		for(int i=0;i<EntryData.length;i++) {
			Object[] SetOb = new Object[WT100ArrivalPlanArrayEntrySourceDataView.RtArrivalPlanArrayEntrySourceDataView().length+1];
			SetOb[0] = false;
		...
		entry_btn.addActionListener(new AbstractAction(){
			public void actionPerformed(ActionEvent e){
				...
				if(null!=ErrMsg && 0<ErrMsg.size()) {
					...
					main_fm.dispose();
					WT100ArrivalPlanArrayEntrySetDataView.ArrivalPlanArrayEntrySetDataView(SetX+10,SetY+10,SetObRt);
				}
			}
		});
	...
	public static ArrayList<String> ErrCheck(Object[][] CheckOb){
		...
		//仕入先マスタ情報取得
		Object[][] SupplierRt= SupplierRt(A00000Main.ClWh,A00000Main.ClCd,SearchSPCd);
		
		//商品変換チェック用に商品変換マスタ情報取得
		Object[][] ItemComversion = WMTools100ItemComversion.ItemComversion(A00000Main.ClCd,ClItemCd);
		...
		for(int i=0;i<CheckOb.length;i++) {
			if(!"".equals((String)CheckOb[i][ColClItemCd])&&0<(int)CheckOb[i][ColPlanQty]) {
				...
				for(int i01=0;i01<ItemComversion.length;i01++) {
					if(((String)CheckOb[i][ColClItemCd]).equals((String)ItemComversion[i01][WMTools100ItemComversion.ColClIemCd])) {
						if(!"".equals((String)ItemComversion[i01][WMTools100ItemComversion.ColItemCd])) {
							UnHitFg = false;
						}
	...
	private static Object[][] SetObRt(Object[][]CheckOb){
		...
		Object[][] SetOb = new Object[EntryCount][WT100ArrivalPlanArrayEntrySetDataView.RtSetDataDefinition().length];
		EntryCount = 0;
		...
		for(int i=0;i<CheckOb.length;i++) {
			if(!"".equals((String)CheckOb[i][ColClItemCd])&&0<(int)CheckOb[i][ColPlanQty]) {
				...
				SetOb[EntryCount][WT100ArrivalPlanArrayEntrySetDataView.ColSetHdClWh]			= (String)CheckOb[i][ColClWh];		//ヘッダ担当倉庫
				SetOb[EntryCount][WT100ArrivalPlanArrayEntrySetDataView.ColSetHdClCd]			= (String)CheckOb[i][ColClCd];		//ヘッダ荷主CD
				SetOb[EntryCount][WT100ArrivalPlanArrayEntrySetDataView.ColSetHdArrNo]		= "";	//ヘッダ入荷予定NO
				SetOb[EntryCount][WT100ArrivalPlanArrayEntrySetDataView.ColSetHdClArrNo]		= (String)CheckOb[i][ColClArrNo];	//ヘッダ荷主予定番号
				SetOb[EntryCount][WT100ArrivalPlanArrayEntrySetDataView.ColSetHdPlanDate]		= (String)CheckOb[i][ColPlanDate];	//ヘッダ入荷予定日
				SetOb[EntryCount][WT100ArrivalPlanArrayEntrySetDataView.ColSetHdActualDate]	= "";	//ヘッダ入荷実績日
				SetOb[EntryCount][WT100ArrivalPlanArrayEntrySetDataView.ColSetHdSpCd]			= (String)CheckOb[i][ColSPCd];		//ヘッダ仕入先CD"}
				SetOb[EntryCount][WT100ArrivalPlanArrayEntrySetDataView.ColSetHdSpName01]		= "";	//ヘッダ仕入先名01
				SetOb[EntryCount][WT100ArrivalPlanArrayEntrySetDataView.ColSetHdSpName02]		= "";	//ヘッダ仕入先名02
				SetOb[EntryCount][WT100ArrivalPlanArrayEntrySetDataView.ColSetHdSpName03]		= "";	//ヘッダ仕入先名03
				SetOb[EntryCount][WT100ArrivalPlanArrayEntrySetDataView.ColSetHdSpPost]		= "";	//ヘッダ仕入先郵便
				SetOb[EntryCount][WT100ArrivalPlanArrayEntrySetDataView.ColSetHdSpAdd01]		= "";	//ヘッダ仕入先住所01
				SetOb[EntryCount][WT100ArrivalPlanArrayEntrySetDataView.ColSetHdSpAdd02]		= "";	//ヘッダ仕入先住所02
				SetOb[EntryCount][WT100ArrivalPlanArrayEntrySetDataView.ColSetHdSpAdd03]		= "";	//ヘッダ仕入先住所03
				SetOb[EntryCount][WT100ArrivalPlanArrayEntrySetDataView.ColSetHdSpTel]		= "";	//ヘッダ仕入先電話
				SetOb[EntryCount][WT100ArrivalPlanArrayEntrySetDataView.ColSetHdArCom01]		= (String)CheckOb[i][ColHdCom01];	//ヘッダコメント1
				SetOb[EntryCount][WT100ArrivalPlanArrayEntrySetDataView.ColSetHdArCom02]		= (String)CheckOb[i][ColHdCom02];	//ヘッダコメント2
				SetOb[EntryCount][WT100ArrivalPlanArrayEntrySetDataView.ColSetHdArCom03]		= (String)CheckOb[i][ColHdCom03];	//ヘッダコメント3
				SetOb[EntryCount][WT100ArrivalPlanArrayEntrySetDataView.ColSetHdFixFg]		= 0;	//ヘッダ状況
				
				SetOb[EntryCount][WT100ArrivalPlanArrayEntrySetDataView.ColSetMsNo]			= "";		//明細番号
				
				SetOb[EntryCount][WT100ArrivalPlanArrayEntrySetDataView.ColSetClItemCd]		= (String)CheckOb[i][ColClItemCd];	//荷主商品コード
				SetOb[EntryCount][WT100ArrivalPlanArrayEntrySetDataView.ColSetJanCd]			= "";		//ソースマーク_BCD
				SetOb[EntryCount][WT100ArrivalPlanArrayEntrySetDataView.ColSetItemMdNo]		= "";		//商品型番
				SetOb[EntryCount][WT100ArrivalPlanArrayEntrySetDataView.ColSetItemName]		= "";		//商品名
				SetOb[EntryCount][WT100ArrivalPlanArrayEntrySetDataView.ColSetlot]				= (String)CheckOb[i][ColLot];	//ロット
				SetOb[EntryCount][WT100ArrivalPlanArrayEntrySetDataView.ColSetExpDate]		= (String)CheckOb[i][ColExpDate];	//消費期限
				SetOb[EntryCount][WT100ArrivalPlanArrayEntrySetDataView.ColSetPlanQty]		= (int)CheckOb[i][ColPlanQty];	//予定数量
				SetOb[EntryCount][WT100ArrivalPlanArrayEntrySetDataView.ColSetActualQty]		= (int)0;	//実績数
				SetOb[EntryCount][WT100ArrivalPlanArrayEntrySetDataView.ColSetActualDate]		= "";		//入荷日
				SetOb[EntryCount][WT100ArrivalPlanArrayEntrySetDataView.ColSetCom01]			= (String)CheckOb[i][ColMsCom01];		//コメント1
				SetOb[EntryCount][WT100ArrivalPlanArrayEntrySetDataView.ColSetCom02]			= (String)CheckOb[i][ColMsCom02];		//コメント2
				SetOb[EntryCount][WT100ArrivalPlanArrayEntrySetDataView.ColUnitType]			= 0;		//荷姿タイプ
				
				
				for(int i01=0;i01<SupplierRt.length;i01++) {
					if(((String)CheckOb[i][ColSPCd]).equals((String)SupplierRt[i01][M100SupplierRt.ColSPCd])) {
						SetOb[EntryCount][WT100ArrivalPlanArrayEntrySetDataView.ColSetHdSpName01]		= (String)SupplierRt[i01][M100SupplierRt.ColSPName01];	//ヘッダ仕入先名01
						SetOb[EntryCount][WT100ArrivalPlanArrayEntrySetDataView.ColSetHdSpName02]		= (String)SupplierRt[i01][M100SupplierRt.ColSPName02];	//ヘッダ仕入先名02
						SetOb[EntryCount][WT100ArrivalPlanArrayEntrySetDataView.ColSetHdSpName03]		= (String)SupplierRt[i01][M100SupplierRt.ColSPName03];	//ヘッダ仕入先名03
						SetOb[EntryCount][WT100ArrivalPlanArrayEntrySetDataView.ColSetHdSpPost]		= (String)SupplierRt[i01][M100SupplierRt.ColSPPost];		//ヘッダ仕入先郵便
						SetOb[EntryCount][WT100ArrivalPlanArrayEntrySetDataView.ColSetHdSpAdd01]		= (String)SupplierRt[i01][M100SupplierRt.ColSPAdd01];	//ヘッダ仕入先住所01
						SetOb[EntryCount][WT100ArrivalPlanArrayEntrySetDataView.ColSetHdSpAdd02]		= (String)SupplierRt[i01][M100SupplierRt.ColSPAdd02];	//ヘッダ仕入先住所02
						SetOb[EntryCount][WT100ArrivalPlanArrayEntrySetDataView.ColSetHdSpAdd03]		= (String)SupplierRt[i01][M100SupplierRt.ColSPAdd03];	//ヘッダ仕入先住所03
						SetOb[EntryCount][WT100ArrivalPlanArrayEntrySetDataView.ColSetHdSpTel]		= (String)SupplierRt[i01][M100SupplierRt.ColSPTel];		//ヘッダ仕入先電話
				...
				for(int i01=0;i01<ItemComversion.length;i01++) {
					if(((String)CheckOb[i][ColClItemCd]).equals((String)ItemComversion[i01][WMTools100ItemComversion.ColClIemCd])) {
						if(!"".equals((String)ItemComversion[i01][WMTools100ItemComversion.ColItemCd])) {
							SetOb[EntryCount][WT100ArrivalPlanArrayEntrySetDataView.ColSetItemCd]	= (String)ItemComversion[i01][WMTools100ItemComversion.ColItemCd];		//商品コード
							SetOb[EntryCount][WT100ArrivalPlanArrayEntrySetDataView.ColUnitType]	= (int)ItemComversion[i01][WMTools100ItemComversion.ColPackingType];	//荷姿タイプ
							
							TgtItemCd.add((String)SetOb[EntryCount][WT100ArrivalPlanArrayEntrySetDataView.ColSetItemCd]);
						}
						i01=ItemComversion.length+1;
					}
				}
		...
		Object[][] ItemMstRt = ItemMstRt(A00000Main.ClGp ,TgtItemCd);
		EntryCount = 0;
		for(int i=0;i<CheckOb.length;i++) {
			if(!"".equals((String)CheckOb[i][ColClItemCd])&&0<(int)CheckOb[i][ColPlanQty]) {
				if(0>=(int)SetOb[EntryCount][WT100ArrivalPlanArrayEntrySetDataView.ColSetPlanQty]) {SetOb[EntryCount][WT100ArrivalPlanArrayEntrySetDataView.ColSetPlanQty] = 0;}	//予定数量
				
				for(int i01=0;i01<ItemMstRt.length;i01++) {
					if(((String)SetOb[EntryCount][WT100ArrivalPlanArrayEntrySetDataView.ColSetItemCd]).equals((String)ItemMstRt[i01][M100ItemMstRt.ColItemCd])) {
						SetOb[EntryCount][WT100ArrivalPlanArrayEntrySetDataView.ColSetJanCd]				= (String)ItemMstRt[i01][M100ItemMstRt.ColJanCd];		//ソースマーク_BCD
						SetOb[EntryCount][WT100ArrivalPlanArrayEntrySetDataView.ColSetItemMdNo]			= (String)ItemMstRt[i01][M100ItemMstRt.ColItemMDNo];		//商品型番
						SetOb[EntryCount][WT100ArrivalPlanArrayEntrySetDataView.ColSetItemName]			= (String)ItemMstRt[i01][M100ItemMstRt.ColItemName01];	//商品名
						...
						switch((int)SetOb[EntryCount][WT100ArrivalPlanArrayEntrySetDataView.ColUnitType]	) {
							case 0:			//バラ
								break;
							case 1:			//カートン
								SetOb[EntryCount][WT100ArrivalPlanArrayEntrySetDataView.ColSetPlanQty]	= GetCtQty*(int)SetOb[EntryCount][WT100ArrivalPlanArrayEntrySetDataView.ColSetPlanQty];	//予定数量
								break;
							case 2:			//ケース
								SetOb[EntryCount][WT100ArrivalPlanArrayEntrySetDataView.ColSetPlanQty]	= GetCsQty*(int)SetOb[EntryCount][WT100ArrivalPlanArrayEntrySetDataView.ColSetPlanQty];	//予定数量
								break;
							case 3:			//パレット
								SetOb[EntryCount][WT100ArrivalPlanArrayEntrySetDataView.ColSetPlanQty]	= GetPlQty*(int)SetOb[EntryCount][WT100ArrivalPlanArrayEntrySetDataView.ColSetPlanQty];	//予定数量
								break;
							default:
								break;
						}
