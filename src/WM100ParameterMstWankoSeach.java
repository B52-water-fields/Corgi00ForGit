...
public class WM100ParameterMstWankoSeach{
	static int SetX;
	static int SetY;
	...
	public static void ParameterMstWankoSeach(int x,int y) {
		...
		final JFrame main_fm = B100FrameParts.FrameCreate(x,y,1200,750,"Corgi00荷主パラメータマスタ（WANKO）検索","");
		...
		JLabel LB_SearchClCd			= B100FrameParts.JLabelSet(	  								  0, 25,130,20,"荷主:"						,11,1);
		final JComboBox  TB_SearchClCd	= B100FrameParts.JComboBoxSet(								130, 25,180,20,B100DefaultVariable.SearchClList[0], 11);
		JLabel LB_SearchWhCd			= B100FrameParts.JLabelSet(	  								  0, 50,130,20,"担当倉庫:"						,11,1);
		final JComboBox  TB_SearchWhCd	= B100FrameParts.JComboBoxSet(								130, 50,180,20,B100DefaultVariable.SearchWhList[0], 11);
		JLabel LB_SearchParaCd			= B100FrameParts.JLabelSet(	  								  0, 75,130,20,"パラメータコード:"			,11,1);
		final JTextField  TB_SearchParaCd	= B100FrameParts.JTextFieldSet(							130, 75,100,20,""							,11,0);
		JLabel LB2_SearchParaCd			= B100FrameParts.JLabelSet(	  								230, 75, 80,20,"と一致"						,11,0);
		JLabel LB_SearchParaCdSeq	= B100FrameParts.JLabelSet(	  									  0,100,130,20,"シーケンシャルNo:"			,11,1);
		final JFormattedTextField TB_SearchParaCdSeqStr= B100FrameParts.JFormattedTextFieldSet(	130,100, 80,20,""							,11,1,"####");
		JLabel LB2_SearchParaCdSeq	= B100FrameParts.JLabelSet(										210,100, 20,20,"～"							,11,2);
		final JFormattedTextField TB_SearchParaCdSeqEnd= B100FrameParts.JFormattedTextFieldSet(	230,100, 80,20,""							,11,1,"####");
		JLabel LB_SearchParaName		= B100FrameParts.JLabelSet(									  0,125,130,20,"パラメータ名:"				,11,1);
		final JTextField  TB_SearchParaName= B100FrameParts.JTextFieldSet(							130,125,100,20,""							,11,0);
		JLabel LB2_SearchParaName		= B100FrameParts.JLabelSet(									230,125, 80,20,"を含む"						,11,0);
		JLabel LB_SearchParaTxtAll		= B100FrameParts.JLabelSet(									  0,150,130,20,"文字設定項目のどれかに:"	,10,1);
		final JTextField  TB_SearchParaTxtAll= B100FrameParts.JTextFieldSet(						130,150,100,20,""							,11,0);
		JLabel LB2_SearchParaTxtAll		= B100FrameParts.JLabelSet(									230,150, 80,20,"を含む"						,11,0);
		...
		//検索ボタン
		JButton SearchBtn = B100FrameParts.BtnSet(130,200,100,20,"検索",11);
		PN_Search.add(SearchBtn);
		
		//条件クリアボタン
		JButton ClearBtn = B100FrameParts.BtnSet( 10,200,100,20,"条件クリア",11);
		...
		SearchBtn.addActionListener(new AbstractAction(){
			public void actionPerformed(ActionEvent e){
				if(RenewFg) {
					...
					String GetSearchClCd			= B100DefaultVariable.SearchClList[1][TB_SearchClCd.getSelectedIndex()];
			...
			public void actionPerformed(ActionEvent e){
				if(RenewFg) {
					...
					if(!"".equals(TgtParaCd) && !"".equals(TgtParaCdSeq)) {
						...
						WM100ParameterMstWankoRenewAndCreate.ParameterMstWankoRenewAndCreate(0,0,TgtWhCd,TgtClCd,TgtParaCd,TgtParaCdSeq);
					}
					RenewFg = true;
				}
			}
		...
		CreateBtn.addActionListener(new AbstractAction(){
			public void actionPerformed(ActionEvent e){
				if(RenewFg) {
					...
					WM100ParameterMstWankoRenewAndCreate.ParameterMstWankoRenewAndCreate(0,0,"","","","");
					RenewFg = true;
				}
			}
		});
			...
			public void actionPerformed(ActionEvent e){
				if(RenewFg) {
					...
					if(null!=Selected && !Selected.equals(Selected.replace(".xlsx", ""))) {
						...
						main_fm.setVisible(false);
						main_fm.dispose();
						WM100ParameterMstWankoExcelEntry.ParameterMstWankoExcelEntry(0,0,Selected);
					}
					RenewFg = true;
				}
			}
		...
		CsvBtn.addActionListener(new AbstractAction(){
			public void actionPerformed(ActionEvent e){
				if(RenewFg) {
					RenewFg = false;
					B100TableControl.TableOutPutCsv("出力先選択","荷主パラメータマスタ検索結果",tb01);
					RenewFg = true;
				}
			}
		});
		...
		ExcelBtn.addActionListener(new AbstractAction(){
			public void actionPerformed(ActionEvent e){
				if(RenewFg) {
					RenewFg = false;
					B100TableControl.TableOutPutExcel("出力先選択","荷主パラメータマスタ検索結果",tb01);
					RenewFg = true;
				}
			}
		});
