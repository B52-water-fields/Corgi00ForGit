...
public class WM100_AdjustReasonMst_00_Search{
	...
	public static void AdjustReasonMstSearch(int x,int y) {
		...
		final JFrame main_fm = B100_FrameParts.FrameCreate(x,y,900,750,"Corgi00在庫調整理由マスタ検索　WM100_AdjustReasonMst_00_Search","");
				...
				if(RenewFg) {
					...
					for(int i=0;i<RowCount;i++) {
						if((boolean)MainFmTableModel.getValueAt(i, 0)) {
							TgtClCd 			= ""+MainFmTableModel.getValueAt(i, M100_AdjustReasonRt.ColClCd+1);
							TgtWhCd 			= ""+MainFmTableModel.getValueAt(i, M100_AdjustReasonRt.ColWhCd+1);
							TgtAdjustReasonCd 	= ""+MainFmTableModel.getValueAt(i, M100_AdjustReasonRt.ColAdjustReasonCd+1);
							KickFg = true;
						}
					}
					if(KickFg) {
