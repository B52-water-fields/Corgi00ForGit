...
public class WM100_DeliveryComversionMst_02_ExcelEntry{
	static int SetX;
	static int SetY;
	...
	public static void DeliveryComversionMstExcelEntry(int x,int y,String TgtFilePath) {
		...
		final JFrame main_fm = B100_FrameParts.FrameCreate(x,y,600,200,"Corgi00届先変換マスタ登録（エクセル）　WM100_DeliveryComversionMst_02_ExcelEntry","");
	...
	public static void DeliveryComversionMstExcelEntryMain(int x,int y,String TgtFilePath,String SheetName) {
		...
		final JFrame main_fm = B100_FrameParts.FrameCreate(x,y,850,800,"Corgi00届先変換マスタ登録（エクセル）　WM100_DeliveryComversionMst_02_ExcelEntry","");
		JLabel userinfo = B100_FrameParts.UserInfo();
		JButton exit_btn = B100_FrameParts.ExitBtn();
		JButton entry_btn = B100_FrameParts.EntryBtn();
	...
	private static void MstEntry(Object[][] CheckOb,String[] TableCol){
		...
		for(int i=0;i<CheckOb.length;i++) {
			...
			if("".equals(GetClGpCD)
				...
				entry_data[EntryCount][12] = "(" + A00000_Main.LoginUserId + ")" + A00000_Main.LoginUserName;	//登録者コード
				entry_data[EntryCount][13] = "(" + A00000_Main.LoginUserId + ")" + A00000_Main.LoginUserName;	//更新者コード
	...
	private static ArrayList<String> ErrCheck(Object[][] CheckOb,String[] TableCol){
		...
		for(int i=0;i<CheckOb.length;i++) {
			String WST = B100_TextControl.Trim(""+CheckOb[i][ColDECD]);
			if(!"".equals(WST)) {
				SearchDECD.add(WST);
			}
		}
		...
		for(int i=0;i<CheckOb.length;i++) {
			...
			if("".equals(GetClGpCD)
				...
				if(!A00000_Main.ClGp.equals(GetClGpCD)) {
					int wint = i+1;
					ErrMsg.add(wint+"行目エラー：("+GetClGpCD+")は現在作業中の荷主グループコードではありません別荷主の設定を投入しようとしていませんか？("+A00000_Main.ClGp+")を設定してください。");
				}
				boolean UnHitFg = true;
				for(int i01=0;i01<DeliveryMstRt.length;i01++) {
					if(GetDECD.equals(""+DeliveryMstRt[i01][M100_DeliveryMstRt.ColDECD]) 
							&& GetDepartmentCd.equals(""+DeliveryMstRt[i01][M100_DeliveryMstRt.ColDepartmentCd]) 
							) {
						UnHitFg = false;
					}
				}
	...
	private static void ErrView(ArrayList<String>ErrMsg) {
		...
		FLD_PATH = A00000_Main.MainFLD+"\\MstControl\\DeliveryComversionMst";
		B100_FolderCheck.FLD_CHECK(FLD_PATH);
		FLD_PATH = A00000_Main.MainFLD+"\\MstControl\\DeliveryComversionMst\\Err";
		B100_FolderCheck.FLD_CHECK(FLD_PATH);
		FLD_PATH = A00000_Main.MainFLD+"\\MstControl\\DeliveryComversionMst\\BK";
		B100_FolderCheck.FLD_CHECK(FLD_PATH);
