...
public class B100_DateTimeControl{
	...
	// ==========================================================================
    //  B100_DateTimeControl（クロノス／時の番人）
    // ==========================================================================
	...
	//以下クロノス、時の魔法書です
	//
    //B100_DateTimeControl.dtm()：現在の日付時刻タイムスタンプを返却します
	//戻り値:Timestamp[2] 
	//
	//B100_DateTimeControl.dtmString(Timestamp[] f0)	:複数のタイムスタンプをrt[i][0]日付　rt[i][1]日付時刻の文字列で返します
	//戻り値:String[][2]
	//
	//B100_DateTimeControl.dtmString2(Timestamp f0)	:単一のタイムスタンプをrt[0]日付　rt[1]日付時刻の文字列で返します
	//戻り値:String[2]
	//
	//B100_DateTimeControl.dtmTimestamp(String[] f0)	：YYYY/MM/DD 又はYYYY/MM/DD HH:MM:SSの配列の文字列で受け取った日付時刻を対応するタイムスタンプの配列にして返却します
	//戻り値:Timestamp[][2]									戻り値rt[i][0]は00：00：00のタイムスタンプ　rt[i][1]は受け取ったタイムスタンプの時刻が返ります
	//														※YYYY/MM/DDを受け取った場合どちらも00：00：00のタイムスタンプになります
	//
	//B100_DateTimeControl.dtmTimestamp2(String f0)	：YYYY/MM/DD 又はYYYY/MM/DD HH:MM:SSの単一の文字列で受け取った日付時刻を対応するタイムスタンプの配列にして返却します
	//戻り値:Timestamp[2]									戻り値rt[0]は00：00：00のタイムスタンプ　rt[1]は受け取ったタイムスタンプの時刻が返ります
	//														※YYYY/MM/DDを受け取った場合どちらも00：00：00のタイムスタンプになります
	//
	//B100_DateTimeControl.dtmHMS(Timestamp[] f0)		:タイムスタンプの各配列要素から対応する時刻をHH:MM:SSの文字列で返却します
	//戻り値:String[]
	//
	//B100_DateTimeControl.dtmHMS2(Timestamp f0)		:単一のタイムスタンプのから対応する時刻をHH:MM:SSの文字列で返却します
	//戻り値:String
	//
	//B100_DateTimeControl.ddif(Timestamp f0,Timestamp f1)	:タイムスタンプFROM～TO受け取って差分日数を返却します
	//戻り値:int
	//
	//B100_DateTimeControl.ndate_before(Timestamp f0,int nd)	:タイムスタンプと遡る日数を受け取ってN日前のタイムスタンプを返却します
	//戻り値:Timestamp
	//
	//B100_DateTimeControl.ndate_after(Timestamp f0,int nd)	:タイムスタンプと先に進む日数を受け取ってN日後のタイムスタンプを返却します
	//戻り値:Timestamp
	//
	//B100_DateTimeControl.nhour_before(Timestamp f0,BigDecimal nd)	:タイムスタンプと遡る時間を受け取ってN時間前のタイムスタンプを返却します※1.5時間前なら1.5渡します
    //戻り値:Timestamp
	//
	//B100_DateTimeControl.nhour_after(Timestamp f0,BigDecimal nd)	:タイムスタンプと先に進む時間を受け取ってN時間後のタイムスタンプを返却します※1.5時間後なら1.5渡します
	//戻り値:Timestamp
	//
	//B100_DateTimeControl.month_str_end(String year,String month)	:YYYYとMMの文字列を受け取って対象年月の月末月初のタイムスタンプを返却します
	//戻り値:Timestamp[2]														rt[0]:対象月初タイムスタンプ
	//																			rt[1]:対象月末タイムスタンプ
	//
	//B100_DateTimeControl.month_str_end2(Timestamp dtm)				:受け取ったタイムスタンプと同年同月の月末月初を返します
	//戻り値:Timestamp[2]														rt[0]:対象月初タイムスタンプ
	//																			rt[1]:対象月末タイムスタンプ
	//
	//B100_DateTimeControl.week_day(Timestamp f1)						:受け取ったタイムスタンプの曜日を対応する数値で返却します
	//戻り値:int																日曜日：1　月曜日：2　火曜日：3　水曜日：4　木曜日：5　金曜日：6　土曜日：7
	//
	//B100_DateTimeControl.hdif(Timestamp f0,Timestamp f1)			:タイムスタンプFROM～TO受け取って差分時間返します
	//戻り値:BigDecimal
	//
	//B100_DateTimeControl.N_MONTH_AFTER_FIRSTDAY_MODE(Timestamp f0,int f1)			:タイムスタンプfromとNケ月後を受け取ってNヶ月後のTimeStamp返します
	//戻り値:Timestamp																			Nヶ月後同日がなければ加算
	//																							EX）2019/01/31,1 ⇒2019/02/31　存在しないので　2019/03/01 hh:mm:ss
	//
	//B100_DateTimeControl.N_MONTH_AFTER_LASTDAY_MODE(Timestamp f0,int f1)			:タイムスタンプfromとNケ月後を受け取ってNヶ月後のTimeStamp返します
	//戻り値:Timestamp																			Nヶ月後同日がなければ減算
	//																							EX）2019/01/31,1 ⇒2019/02/31　存在しないので　2019/02/28
	//
	//B100_DateTimeControl.DateStyleRt(String DateString)		 						:日付文字列yyyymmdd(8桁数字)又は　yyyy/m/dを受け取ってyyyy/mm/ddの文字列にして返却する
	//戻り値:String																				↓DateFormatの下位互換（でもこの2パターンだとわかっていればこっち使う）
	//
	//B100_DateTimeControl.DateFormat(String CSt)										:日付項目yyyy/mm/ddにする　日付っぽい形式の文字列をyyyy/mm/ddの文字列にします
	...
	public static String DateStyleRt(String DateString) {
		...
		if(8==(""+DateString).length()&& 8==(B100_TextControl.num_only_String(""+DateString)).length()) {
	...
	public static String DateFormat(String CSt) {
		...
		if(!(CSt.equals(CSt.replace("/", "")))) {
			...
			CSt = B100_TextControl.num_only_String(CSt);
			if(8==CSt.length()) {
				//yyyymmddと判断
		...
		if(!"".equals(rt)) {
			...
			if(10==rt.length()) {
				...
				if(!"".equals(rt)) {rt=B100_DateTimeControl.dtmString2(B100_DateTimeControl.dtmTimestamp2(rt)[0])[0];}
			}else {
				rt = "";
			}
		}
