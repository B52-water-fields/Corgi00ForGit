...
public class WT100ArrivalPlanSomeEntry{
	static int SetX;
	static int SetY;
	...
	public static void ArrivalPlanSomeEntry(int x,int y) {
		...
		entry_btn.addActionListener(new AbstractAction(){
			public void actionPerformed(ActionEvent e){
				...
				String[][] SetData = new String[RowCount][WT100ArrivalPlanArrayEntrySourceDataView.RtArrivalPlanArrayEntrySourceDataView().length];
				
				for(int i=0;i<RowCount;i++) {
					SetData[i][WT100ArrivalPlanArrayEntrySourceDataView.ColClWh]		= GetClWh;		//担当倉庫
					SetData[i][WT100ArrivalPlanArrayEntrySourceDataView.ColClCd]		= GetClCd;		//荷主CD
					SetData[i][WT100ArrivalPlanArrayEntrySourceDataView.ColClArrNo]	= "";			//荷主予定番号
					SetData[i][WT100ArrivalPlanArrayEntrySourceDataView.ColPlanDate]	= GetPlanDate;	//入荷予定日
					SetData[i][WT100ArrivalPlanArrayEntrySourceDataView.ColClItemCd]	= "";			//荷主商品CD
					SetData[i][WT100ArrivalPlanArrayEntrySourceDataView.ColPlanQty]	= "";			//数量
					SetData[i][WT100ArrivalPlanArrayEntrySourceDataView.ColSPCd]		= GetSpCd;		//仕入先CD
					SetData[i][WT100ArrivalPlanArrayEntrySourceDataView.ColLot]		= "";			//ロット
					SetData[i][WT100ArrivalPlanArrayEntrySourceDataView.ColExpDate]	= "";			//賞味期限
					SetData[i][WT100ArrivalPlanArrayEntrySourceDataView.ColHdCom01]	= "";			//ヘッダコメント01
					SetData[i][WT100ArrivalPlanArrayEntrySourceDataView.ColMsCom01]	= "";			//明細コメント01
					
					if(i<ArrayGetClArrNo.length) {
						SetData[i][WT100ArrivalPlanArrayEntrySourceDataView.ColClArrNo]	= ArrayGetClArrNo[i];			//荷主予定番号
					}
					if(i<ArrayGetClItemCd.length) {
						SetData[i][WT100ArrivalPlanArrayEntrySourceDataView.ColClItemCd]	= ArrayGetClItemCd[i];			//荷主商品CD
					}
					if(i<ArrayGetlot.length) {
						SetData[i][WT100ArrivalPlanArrayEntrySourceDataView.ColLot]		= ArrayGetlot[i];				//ロット
					}
					if(i<ArrayGetExpDate.length) {
						SetData[i][WT100ArrivalPlanArrayEntrySourceDataView.ColExpDate]	= ArrayGetExpDate[i];			//賞味期限
					}
					if(i<ArrayGetPlanQty.length) {
						SetData[i][WT100ArrivalPlanArrayEntrySourceDataView.ColPlanQty]	= ArrayGetPlanQty[i];			//数量
					}
					if(i<ArrayGetHdCom.length) {
						SetData[i][WT100ArrivalPlanArrayEntrySourceDataView.ColHdCom01]	= ArrayGetHdCom[i];				//ヘッダコメント
					}
					if(i<ArrayGetMsCom.length) {
						SetData[i][WT100ArrivalPlanArrayEntrySourceDataView.ColMsCom01]	= ArrayGetMsCom[i];				//明細コメント
					}
				}
				if(0<RowCount) {
					SetX=main_fm.getX();
					...
					main_fm.dispose();
					WT100ArrivalPlanArrayEntrySourceDataView.ArrivalPlanArrayEntrySourceDataView(SetX+10,SetY+10,SetData);
				}
			}
		});
