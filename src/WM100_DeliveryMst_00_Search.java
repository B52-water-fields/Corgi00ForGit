...
public class WM100_DeliveryMst_00_Search{
	static int SetX;
	static int SetY;
	...
	public static void DeliveryMstSearch(int x,int y) {
		...
		JLabel LB2_SearcNotJis			= B100_FrameParts.JLabelSet(700,100, 50,20,""				,11,0);
		...
		SearchBtn.addActionListener(new AbstractAction(){
			public void actionPerformed(ActionEvent e){
				if(RenewFg) {
					...
					GetSearchPrefecturesCd	= B100_TextControl.Trim(GetSearchPrefecturesCd);
					GetSearchMunicipalityCd	= B100_TextControl.Trim(GetSearchMunicipalityCd);
			...
			public void actionPerformed(ActionEvent e){
				if(RenewFg) {
					...
					if(!"".equals(TgtDECD) && !"".equals(TgtDepartmentCd)) {
						...
						main_fm.dispose();
						WM100_DeliveryMst_01_RenewAndCreate.DeliveryMstRenewAndCreate(0,0,TgtDECD,TgtDepartmentCd);
					}
					RenewFg = true;
				}
			}
		...
		CreateBtn.addActionListener(new AbstractAction(){
			public void actionPerformed(ActionEvent e){
				...
				main_fm.setVisible(false);
				main_fm.dispose();
				WM100_DeliveryMst_01_RenewAndCreate.DeliveryMstRenewAndCreate(0,0,"","");
			}
		});
		...
		CsvBtn.addActionListener(new AbstractAction(){
			public void actionPerformed(ActionEvent e){
				if(RenewFg) {
					RenewFg = false;
					B100_TableControl.TableOutPutCsv("出力先選択","届先マスタ検索結果",tb01);
					RenewFg = true;
				}
			}
		});
		...
		ExcelBtn.addActionListener(new AbstractAction(){
			public void actionPerformed(ActionEvent e){
				if(RenewFg) {
					RenewFg = false;
					B100_TableControl.TableOutPutExcel("出力先選択","届先マスタ検索結果",tb01);
					RenewFg = true;
				}
			}
		});
			...
			public void actionPerformed(ActionEvent e){
				if(RenewFg) {
					...
					if(null!=Selected && !Selected.equals(Selected.replace(".xlsx", ""))) {
						...
						main_fm.setVisible(false);
						main_fm.dispose();
						WM100_DeliveryMst_02_ExcelEntry.DeliveryMstExcelEntry(0,0,Selected);
					}
					RenewFg = true;
				}
			}
