...
public class WM100_PostMst_03_CreateSum{
	static int SetX;
	static int SetY;
	public static void PostMstCreateSum(int x,int y) {
		...
		JLabel LB_EntryPost = B100_FrameParts.JLabelSet(				 10,40,100,20,"郵便番号"  ,11,2);
		JLabel LB_EntryPref = B100_FrameParts.JLabelSet(				120,40,100,20,"県"        ,11,2);
		JLabel LB_EntryMunic01 = B100_FrameParts.JLabelSet(			230,40,200,20,"市区町村"  ,11,2);
		JLabel LB_EntryMunic02 = B100_FrameParts.JLabelSet(			440,40,200,20,"町丁目"    ,11,2);
		JLabel LB_EntryMunicipalityCd = B100_FrameParts.JLabelSet(	650,40,100,20,"市区町村CD",11,2);
		...
		//スクロール用設定
		JScrollPane Scpn_EntryPost = B100_FrameParts.JScrollPaneSet(				 10,60,100,500,TB_EntryPost);			//郵便番号
		JScrollPane Scpn_EntryPref = B100_FrameParts.JScrollPaneSet(				120,60,100,500,TB_EntryPref);			//県
		JScrollPane Scpn_EntryMunic01 = B100_FrameParts.JScrollPaneSet(			230,60,200,500,TB_EntryMunic01);		//市区町村
		JScrollPane Scpn_EntryMunic02 = B100_FrameParts.JScrollPaneSet(			440,60,200,500,TB_EntryMunic02);		//町丁目
		JScrollPane Scpn_EntryMunicipalityCd = B100_FrameParts.JScrollPaneSet(	650,60,100,500,TB_EntryMunicipalityCd);	//市区町村CD
		...
		//郵便DLデータ取込ボタン
		JButton DlEntryBtn = B100_FrameParts.BtnSet(10,580,100,20,"DL（Ken_All）",9);
		...
		DlEntryBtn.addActionListener(new AbstractAction(){
			public void actionPerformed(ActionEvent e){
				...
				String selected = B100_FileSelect.FileSelect("ファイル選択",file_type,file_type_name);
				
				if(null!=selected && !"".equals(selected)) {
					String[][] CSVRead = B100_TextRead.CSVRead(selected,null);
					...
					if(0<CSVRead.length && 10<=CSVRead[0].length) {
						...
						for(int i=0;i<CSVRead.length;i++) {
							...
							GetEntryPost[i]				 = B100_TextControl.Trim(""+CSVRead[i][2]);
							GetEntryPref[i]				 = B100_TextControl.Trim(""+CSVRead[i][6]);
							GetEntryMunic01[i]			 = B100_TextControl.Trim(""+CSVRead[i][7]);
							GetEntryMunic02[i]			 = B100_TextControl.Trim(""+CSVRead[i][8]);
							GetEntryMunicipalityCd[i]	 = B100_TextControl.Trim(""+CSVRead[i][0]);
	...
	private static void PostSet(int x,int y,String[] GetEntryPost,String[] GetEntryPref,String[] GetEntryMunic01,String[] GetEntryMunic02,String[] GetEntryMunicipalityCd) {
		...
		if(NonErr) {
			for(int i=0;i<GetEntryPost.length;i++) {
				GetEntryPost[i] = B100_TextControl.num_only_String(GetEntryPost[i]);
		...
		if(NonErr) {
			...
			entry_btn.addActionListener(new AbstractAction(){
				public void actionPerformed(ActionEvent e){
					...
					for(int i=0;i<GetEntryPost.length;i++) {
						judg_data[i][0] = B100_TextControl.Trim(GetEntryPost[i]);	//郵便番号
						
						entry_data[i][0] = B100_TextControl.Trim(GetEntryPost[i]);			//郵便番号
						entry_data[i][1] = B100_TextControl.Trim(GetEntryPref[i]);			//県
						entry_data[i][2] = B100_TextControl.Trim(GetEntryMunic01[i]);			//市区町村
						entry_data[i][3] = B100_TextControl.Trim(GetEntryMunic02[i]);			//町丁目
						entry_data[i][4] = B100_TextControl.Trim(GetEntryMunicipalityCd[i]);	//市区町村CD
					}
