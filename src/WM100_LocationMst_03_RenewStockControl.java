...
public class WM100_LocationMst_03_RenewStockControl{
	...
	*/
	public static void LocationMstRenewStockControl(
		...
		for(String[] GetVal : SetLoc.values()){
			...
			for(int i01=0;i01<B100_DefaultVariable.LocType[1].length;i01++) {
				if(GetSetType.equals(B100_DefaultVariable.LocType[1][i01])) {
					SetPickTgtLocFg = Integer.parseInt(B100_DefaultVariable.LocType[3][i01]);
				}
			}
			...
			if(null!=GetSt) {
				...
				for(int i01=0;i01<B100_DefaultVariable.LocType[1].length;i01++) {
					if(GetNowType.equals(B100_DefaultVariable.LocType[1][i01])) {
						NowPickTgtLocFg = Integer.parseInt(B100_DefaultVariable.LocType[3][i01]);
					}
				}
	...
	private static void LocationRenew(HashMap<String,String[]> SetLoc) {
		...
		for(String[] GetVal : SetLoc.values()){
			...
			entry_data[SetRow][7]	= "(" + A00000_Main.LoginUserId + ")" + A00000_Main.LoginUserName; 	//登録者
			entry_data[SetRow][8]	= "(" + A00000_Main.LoginUserId + ")" + A00000_Main.LoginUserName; 	//更新者
	...
	private static void StockRenew(String GetSetClCd,String GetSetWhCd,String GetSetLoc,String GetSetType,int SetPickTgtLocFg) {
		...
		for(int i=0;i<StockRt.length;i++) {
			...
			int GetQty				= (int)StockRt[i][T100_StockRt.ColQty];				//数量
			int GetShipPlanQty		= (int)StockRt[i][T100_StockRt.ColShipPlanQty];		//引当済数
			int GetPossibleQty		= (int)StockRt[i][T100_StockRt.ColPossibleQty];		//出荷可能数
			...
			entry_data[i][11]	= "(" + A00000_Main.LoginUserId + ")" + A00000_Main.LoginUserName;	//更新者
		...
		if(0<entry_data.length) {
			...
			if(null!=GetShipPlanQtyMSG && 0<GetShipPlanQtyMSG.size()) {
				GetShipPlanQtyMSG = B100_ArrayListControl.ArryListStringUniqueList(GetShipPlanQtyMSG);
