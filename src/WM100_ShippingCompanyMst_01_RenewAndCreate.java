...
public class WM100_ShippingCompanyMst_01_RenewAndCreate{
	static int SetX;
	static int SetY;
	...
	public static void ShippingCompanyMstRenewAndCreate(int x,int y,String ShippingCompanyCd) {
		...
		final JTextField TB_Add03					= B100_FrameParts.JTextFieldSet(100,215,250,20,"",11,0);	//運送会社住所3
		final JTextField TB_Tel						= B100_FrameParts.JTextFieldSet(100,240,100,20,"",11,0);	//運送会社電話
		final JTextField TB_Fax						= B100_FrameParts.JTextFieldSet(100,265,100,20,"",11,0);	//運送会社FAX
		final JTextField TB_Mail					= B100_FrameParts.JTextFieldSet(100,290,100,20,"",11,0);	//運送会社MAIL
		final JTextField TB_Com01					= B100_FrameParts.JTextFieldSet(100,315,100,20,"",11,0);	//コメント1
		final JTextField TB_Com02					= B100_FrameParts.JTextFieldSet(100,340,100,20,"",11,0);	//コメント2
		final JTextField TB_Com03					= B100_FrameParts.JTextFieldSet(100,365,100,20,"",11,0);	//コメント3
		final JComboBox TB_ShimeDate				= B100_FrameParts.JComboBoxSet( 100,390,100,20,B100_DefaultVariable.ShimeDateList[0],	11);	//締日
		final JComboBox TB_ShimeBasis				= B100_FrameParts.JComboBoxSet( 100,415,150,20,B100_DefaultVariable.DeliFeeNorm[0],	11);	//請求基準
		final JTextField TB_EntryDate				= B100_FrameParts.JTextFieldSet(100,440,250,20,"",11,0);	//データ登録日時
		final JTextField TB_UpdateDate				= B100_FrameParts.JTextFieldSet(100,465,250,20,"",11,0);	//データ更新日時
		final JTextField TB_EntryUser				= B100_FrameParts.JTextFieldSet(100,490,250,20,"",11,0);	//登録者コード
		final JTextField TB_UpdateUser				= B100_FrameParts.JTextFieldSet(100,515,250,20,"",11,0);	//更新者コード
		final JTextField TB_PTMSCD					= B100_FrameParts.JTextFieldSet(100,540,100,20,"",11,0);	//基幹SYS傭車CD
		final JComboBox  TB_ExportDataType			= B100_FrameParts.JComboBoxSet( 100,565,200,20,B100_DefaultVariable.HaisyaDataLayoutPt[0],	11);	//データ抽出Type
		...
		if(!"".equals(ShippingCompanyCd)) {
			...
			if(0<ShippingCompanyMstRt.length) {
				...
				for(int i=0;i<B100_DefaultVariable.ShimeDateList[1].length;i++) {
					if((""+B100_DefaultVariable.ShimeDateList[1][i]).equals(""+ShippingCompanyMstRt[0][M100_ShippingCompanyMstRt.ColShimeDate])){
						TB_ShimeDate.setSelectedIndex(i);
					}
				}
				for(int i=0;i<B100_DefaultVariable.DeliFeeNorm[1].length;i++) {
					if((""+B100_DefaultVariable.DeliFeeNorm[1][i]).equals(""+ShippingCompanyMstRt[0][M100_ShippingCompanyMstRt.ColShimeBasis])){
						TB_ShimeBasis.setSelectedIndex(i);
					}
				}
				for(int i=0;i<B100_DefaultVariable.HaisyaDataLayoutPt[1].length;i++) {
					if((""+B100_DefaultVariable.HaisyaDataLayoutPt[1][i]).equals(""+ShippingCompanyMstRt[0][M100_ShippingCompanyMstRt.ColExportDataType])){
		...
		entry_btn.addActionListener(new AbstractAction(){
			public void actionPerformed(ActionEvent e){
				if(RenewFg) {
					...
					String GetShimeDate				= ""+B100_DefaultVariable.ShimeDateList[1][TB_ShimeDate.getSelectedIndex()];	//締日
					...
					String GetExportDataType		= ""+B100_DefaultVariable.HaisyaDataLayoutPt[1][TB_ExportDataType.getSelectedIndex()];	//データ抽出Type
					...
					GetShippingCompanyName01	= B100_TextControl.Trim(GetShippingCompanyName01);
					GetShippingCompanyName02	= B100_TextControl.Trim(GetShippingCompanyName02);
					GetShippingCompanyName03	= B100_TextControl.Trim(GetShippingCompanyName03);
					...
					GetExportDataType			= B100_TextControl.Trim(GetExportDataType);
					...
					GetShimeDate				= B100_TextControl.num_only_String(GetShimeDate);
					GetShimeBasis				= B100_TextControl.num_only_String(GetShimeBasis);
					...
					if(!"".equals(GetShippingCompanyName01)) {
						if("".equals(GetShippingCompanyCd)) {
							GetShippingCompanyCd = M100_ShippingCompanyMstRt.NewShipingCompanyCdGet(1)[0];
						}
						...
						B100_DefaultVariable.ShippingCompanyList();		//運送会社一覧生成
						...
						main_fm.dispose();
						WM100_ShippingCompanyMst_01_RenewAndCreate.ShippingCompanyMstRenewAndCreate(0, 0,GetShippingCompanyCd);
					}else {
