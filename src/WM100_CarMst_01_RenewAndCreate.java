...
public class WM100_CarMst_01_RenewAndCreate{
	static int SetX;
	static int SetY;
	...
	public static void CarMstRenewAndCreate(int x,int y,String TgtWhCd,String TgtShippingCompanyCd,String TgtCarCd) {
		...
		final JFrame main_fm = B100_FrameParts.FrameCreate(x,y,500,500,"Corgi00車輛登録・修正　WM100_CarMst_01_RenewAndCreate","");
		...
		final JTextField TB_UpdateDate			= B100_FrameParts.JTextFieldSet(100,265,250,20,"",11,0);	//データ更新日時
		final JTextField TB_EntryUser			= B100_FrameParts.JTextFieldSet(100,290,250,20,"",11,0);	//登録者コード
		final JTextField TB_UpdateUser			= B100_FrameParts.JTextFieldSet(100,315,250,20,"",11,0);	//更新者コード
		final JComboBox  TB_DelFg				= B100_FrameParts.JComboBoxSet( 100,340,150,20,B100_DefaultVariable.DelList[0],	11);
		...
		if(!"".equals(TgtWhCd) && !"".equals(TgtShippingCompanyCd) && !"".equals(TgtCarCd)) {
			...
			if(0<CarMstRt.length) {
				for(int i=0;i<B100_DefaultVariable.WhList[1].length;i++) {
					if((""+CarMstRt[0][M100_CarMstRt.ColWHCD]).equals(""+B100_DefaultVariable.WhList[1][i])) {
						TB_WHCD.setSelectedIndex(i);
					}
				}
				for(int i=0;i<B100_DefaultVariable.ShippingCompanyList[1].length;i++) {
					if((""+CarMstRt[0][M100_CarMstRt.ColShippingCompanyCd]).equals(""+B100_DefaultVariable.ShippingCompanyList[1][i])) {
						TB_ShippingCompanyCd.setSelectedIndex(i);
					}
				}
		...
		JButton Check_btn = B100_FrameParts.BtnSet(B100_FrameParts.Width-140, 90,100,20,"CD確認",11);			//車輛CDの存在確認
		main_fm.add(Check_btn);
		
		JButton CdRenew_btn = B100_FrameParts.BtnSet(B100_FrameParts.Width-140,115,100,20,"CD修正Mode",11);		//車輛CDの存在確認
		...
		NewCreate_btn.addActionListener(new AbstractAction(){
			public void actionPerformed(ActionEvent e){
				...
				main_fm.dispose();
				WM100_CarMst_01_RenewAndCreate.CarMstRenewAndCreate(0, 0,"","","");
			}
		});
		...
		Check_btn.addActionListener(new AbstractAction(){
			public void actionPerformed(ActionEvent e){
				...
				String GetShippingCompanyCd	= ""+B100_DefaultVariable.ShippingCompanyList[1][TB_ShippingCompanyCd.getSelectedIndex()];
				...
				GetCarCd				= B100_TextControl.only1byte_String(GetCarCd);
		...
		entry_btn.addActionListener(new AbstractAction(){
			public void actionPerformed(ActionEvent e){
				if(RenewFg) {
					...
					GetShippingCompanyCd	= B100_TextControl.Trim(GetShippingCompanyCd);
					GetCarCd				= B100_TextControl.Trim(GetCarCd);
					GetCarName01			= B100_TextControl.Trim(GetCarName01);
					GetCarName02			= B100_TextControl.Trim(GetCarName02);
					GetCarName03			= B100_TextControl.Trim(GetCarName03);
					GetDriverCd				= B100_TextControl.Trim(GetDriverCd);
					...
					if(KickFg) {
						...
						if(KickFg) {
							...
							main_fm.setVisible(false);
							main_fm.dispose();
							WM100_CarMst_01_RenewAndCreate.CarMstRenewAndCreate(0, 0,GetWHCD,GetShippingCompanyCd,GetCarCd);
						}
					}
